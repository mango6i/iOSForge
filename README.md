...
“最近构建”支持逐条勾选、全选已加载的已结束记录、只选失败/取消、加载更早记录，以及确认后批量删除。删除记录会连同对应日志和全部云端产物一起删除，源码、提交历史和本地下载保留。只作用于当前仓库/分支的 `build.yml`，不操作 Pages 或环境检查；进行中或状态已变化的任务不删除。网络/权限错误会停止后续操作，需刷新核对结果。暂未启用定时删除记录；产物保留 14 天的现有设置不等于构建记录也会自动消失。
