...
4. 构建成功后，成品会按 `Download/项目名称/运行编号/真实文件名` 保存；网页直接显示和下载 `.ipa`、`.deb`、`.dylib`，没有外层 ZIP。
5. 不再需要某个版本时点击文件旁的“删除”；只删除选中的真实文件，空目录会自动消失。彻底删除一个项目时，“项目源码管理”会一次性删除 `sources/项目名称/` 和 `Download/项目名称/`，其他项目不受影响。

“最近构建”支持逐条勾选、全选已加载的已结束记录、只选失败/取消、加载更早记录，以及确认后批量删除。删除记录会删除对应 Actions 日志，但不会删除 `Download` 中的真实产物或源码。只作用于当前仓库/分支的 `build.yml`，不操作 Pages 或环境检查；进行中或状态已变化的任务不删除。网络/权限错误会停止后续操作，需刷新核对结果。暂未启用定时删除记录。

真实产物是普通仓库文件，不会按 14 天自动过期，单文件限制为 95 MiB。公开仓库中的源码和 `Download` 成品任何人都能看到；普通删除只改变当前分支，不会抹掉旧 Git 提交中的历史副本。长期保留大量 IPA 会增加仓库体积，建议下载留存后及时清理不再需要的版本。
...
- `iosforge/`：构建工具源码；
- `web/`：网页控制台；
- `Download/项目名/运行编号/`：首次构建成功后自动创建，保存真实 IPA、DEB、DYLIB；文件删完后空目录自动消失；
