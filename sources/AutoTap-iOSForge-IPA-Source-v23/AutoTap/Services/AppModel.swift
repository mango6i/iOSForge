...
final class AppModel: ObservableObject {
    ...
    private static let releaseAPI = URL(string: "https://api.github.com/repos/mango6i/iOS-AutoTap/releases/latest")!
    ...
    func finishRecordingCapture() {
        ...
        guard !recordedActions.isEmpty else {
            bannerMessage = "尚未录制到点击，请先在悬浮条外点击目标应用。"
            return
        }
    ...
    func saveRecordedScript() {
        ...
        let id = store.addRecordedProfile(name: finalName, actions: recordedActions)
        ...
        systemOverlay.hide()
        BackgroundKeeper.shared.stop()
        bannerMessage = "录制脚本“\(finalName)”已保存，可在录制模式的“设置与已保存脚本”中编辑或开启回放。"
    }
    ...
    private func appendRecordedTap(
        normalizedX: CGFloat,
        normalizedY: CGFloat,
        ...
        guard recordedActions.count < 200 else {
            bannerMessage = "单个录制脚本最多保存 200 个点击。"
            return
        }
    ...
    func checkForUpdates() {
        ...
        request.setValue("AutoTap-iOS/2.4.0", forHTTPHeaderField: "User-Agent")
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            DispatchQueue.main.async {
                guard let self else { return }
                self.isCheckingForUpdates = false
                if let error {
                    self.bannerMessage = "检查更新失败：\(error.localizedDescription)"
                    return
                }
                guard let http = response as? HTTPURLResponse,
                      ...
                      let rawURL = object["html_url"] as? String,
                      let releaseURL = URL(string: rawURL) else {
                    self.bannerMessage = "暂时无法读取更新信息，请稍后重试。"
                    return
                }
                let newest = rawTag.trimmingCharacters(in: CharacterSet(charactersIn: "vV"))
                let current = Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "2.4.0"
                if self.isVersion(newest, newerThan: current) {
                    ...
                    self.bannerMessage = "发现新版本 \(rawTag)"
                } else {
