        ...
        ScrollView {
            VStack(spacing: 16) {
                ...
                if let index = selectedActionIndex {
                    ActionInspector(actions: profile.actions, index: index).environmentObject(model)
                }
                runSettings
            }
            .padding(16)
        ...
        VStack(alignment: .leading, spacing: 14) {
            HStack(spacing: 8) {
                ...
                if total > 1 {
                    HStack(spacing: 16) {
                        if hasPrevious {
                            navButton(title: "上一个", symbol: "chevron.left", symbolFirst: true, action: goPrevious)
                        }
                        if hasNext {
                            navButton(title: "下一个", symbol: "chevron.right", symbolFirst: false, action: goNext)
                        }
                    }
                }
            }
