...
struct ModeEditorView: View {
    ...
    private var editorCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            ControlBar(
                ...
                play: { engine.isActive ? model.stop() : model.startSelectedProfile() },
    ...
    private var systemHint: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: "shield.lefthalf.fill").foregroundColor(AppTheme.warning)
            Text(model.runDestination == .system
                 ? "系统实验模式会在倒计时后打开目标 App。切回 AutoTap 会立即停止。"
                 : "当前为 App 内预览，只显示目标执行动画，不会点击其他 App。")
                .font(.footnote)
                .foregroundColor(.secondary)
        }
        .appCard()
