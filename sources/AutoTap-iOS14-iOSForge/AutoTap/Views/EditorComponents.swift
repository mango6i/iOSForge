...
private struct SwipeEndHandle: View {
    ...
    var body: some View {
        ZStack {
            ...
            Image(systemName: "arrow.up.right").font(.caption).fontWeight(.bold).foregroundColor(.white)
        }
...
struct ControlBar: View {
    ...
    var body: some View {
        HStack(spacing: 7) {
            toolButton(symbol: isRunning ? "stop.fill" : "play.fill", color: isRunning ? .red : AppTheme.accent, action: play)
