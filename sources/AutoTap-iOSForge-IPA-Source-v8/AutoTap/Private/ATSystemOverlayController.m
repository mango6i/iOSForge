...
- (void)handlePhysicalHIDEvent:(IOHIDEventRef)event {
    // A hosted accessibility window is rendered by SpringBoard. Even while
    // AutoTap itself is foreground, its physical touch stream is not
    // guaranteed to be delivered to that hosted context. Route matching
    // touches in every application state; the synthetic sender ID prevents
    // the monitor from recursively forwarding the injected copy.
...
- (BOOL)showWithPoints:(NSArray<NSValue *> *)points
    ...
    // v8 keeps all hit targets in the HUD. The legacy middle window remains
    // hidden so a failed third hosting registration cannot disable controls.
...
- (void)showPointEditorAtIndex:(NSInteger)index {
    ...
    unit.frame = CGRectMake(20, 253, panelWidth - 40, 38);
...
- (void)editorSave:(id)sender {
    ...
    NSInteger repeats = MAX([[self.editorRepeatField titleForState:UIControlStateNormal] integerValue], 1);
