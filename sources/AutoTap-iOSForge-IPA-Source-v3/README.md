...
- 首页悬浮控制层：单点、多点分别开启；支持启动、停止、添加与删除目标，控制条可拖动。
- 跨应用执行：通过私有 IOKit HID 接口向前台应用发送触摸，并用后台音频维持任务。
...
- 跨应用点击使用私有 API，只面向你拥有并获授权的 TrollStore / 越狱设备；它不能上架 App Store。
- TrollStore 安装成功不代表设备固件一定允许 HID 派发。系统模式依赖私有权限 `com.apple.private.hid.client.event-dispatch`；某些系统、签名器或安装方式会移除/拒绝该权限。
- iOS 不允许普通 App 在其他 App 上方显示真正的跨进程悬浮控制条。悬浮控制层显示在 AutoTap 内；执行时采用“倒计时 → 打开目标 App → 后台执行”的流程，回到 AutoTap 即安全停止。
...
> iOSForge 网页当前标注 iOS 15+，本工程自身的 Deployment Target 是 iOS 14.0；项目只使用 iOS 14 可用的公开 UI API。最终能否在具体固件执行跨应用点击，仍需真机验证私有 API 与签名权限。

## TrollStore / 私有权限

仓库根目录提供 `entitlements.plist`，Xcode 工程也引用 `AutoTap/AutoTap.entitlements`。如果你的签名流程允许保留自定义 entitlements，请使用该文件。普通免费自签通常无法授予私有 HID 权限，此时 App 仍能打开和编辑脚本，但可能无法产生跨应用触摸。
...
6. 要停止任务，切回 AutoTap；最长运行时间到达后也会自动停止。
