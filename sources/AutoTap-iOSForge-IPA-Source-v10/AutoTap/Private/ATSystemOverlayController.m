...
- (void)handlePhysicalHIDEvent:(IOHIDEventRef)event {
    ...
    uint64_t timestamp = _getHIDTimestamp(event);
    if (ATTouchDispatcherIsSyntheticTimestamp(timestamp)) { return; }
    ...
    if (began) { [self beginDirectTouchAtPoint:point]; }
    else if (touching) { [self moveDirectTouchToPoint:point]; }
    _physicalTouchActive = touching;
    if (ended) { [self endDirectTouchAtPoint:_lastPhysicalTouchPoint cancelled:NO]; }
}

- (UIView *)hudHitViewAtScreenPoint:(CGPoint)point {
    if (!self.hudWindow || self.hudWindow.hidden) { return nil; }
    CGPoint local = [self.hudWindow convertPoint:point fromWindow:nil];
...
- (void)beginDirectTouchAtPoint:(CGPoint)point {
    ...
    if (markerIndex != NSNotFound && !self.running) {
...
- (void)activateDirectControl:(UIView *)control atScreenPoint:(CGPoint)point {
    ...
    CGPoint hudPoint = [self.hudWindow convertPoint:point fromWindow:nil];
    CGPoint local = [control convertPoint:hudPoint fromView:self.hudController.view];
...
- (void)updateWithPoints:(NSArray<NSValue *> *)points
    ...
    if (!NSThread.isMainThread) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self updateWithPoints:points actionSettings:actionSettings multiple:multiple selectedIndex:selectedIndex activeIndex:activeIndex markerScale:markerScale controlScale:controlScale running:running countdownSeconds:countdownSeconds];
        });
        return;
    }
...
- (void)rebuildOverlayContent {
    ...
    for (NSInteger index = 0; index < self.points.count; index++) {
        ...
        hit.tag = index;
        hit.userInteractionEnabled = !self.running;
        ...
        // Keep every physical control in the topmost HUD context. A separate
        // lower interaction window can be shadowed by the transparent HUD and
        // never receive touches, even while this app is foreground.
...
- (void)buildToolbar {
    ...
    // Dragging can begin anywhere on the panel. A quick tap still reaches the
    // button; movement makes the pan recognizer take over and cancels TouchUp.
    UIPanGestureRecognizer *panelPan = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(toolbarDragged:)];
...
- (void)updateCountdownHUD {
    ...
    if (!self.countdownBadge) {
        ...
        badge.backgroundColor = [UIColor colorWithWhite:0.04 alpha:0.92];
        ...
        caption.textColor = UIColor.systemGray2Color;
...
- (UIButton *)editorValueButtonWithTitle:(NSString *)title tag:(NSInteger)tag frame:(CGRect)frame {
    ...
    [button addTarget:self action:@selector(editorFieldTapped:) forControlEvents:UIControlEventTouchUpInside];
    return button;
}

- (void)showPointEditorAtIndex:(NSInteger)index {
    if (self.running || index < 0 || index >= self.actionSettings.count) { return; }
    ...
    [backdrop addTarget:self action:@selector(editorCancel:) forControlEvents:UIControlEventTouchUpInside];
    ...
    UILabel *repeatTip = [self editorLabelWithText:@"执行次数填 0 = 无限次，直到手动暂停或停止"
    ...
    for (NSInteger idx = 0; idx < keys.count; idx++) {
        ...
        [key addTarget:self action:@selector(editorKeyTapped:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [cancel addTarget:self action:@selector(editorCancel:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [save addTarget:self action:@selector(editorSave:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)editorSave:(id)sender {
    ...
    NSInteger duration = MAX([[self.editorDurationField titleForState:UIControlStateNormal] integerValue], 40);
...
- (void)markerTapped:(UITapGestureRecognizer *)gesture {
    ...
    [self updateOverlayContentInPlace];
    [self showPointEditorAtIndex:self.selectedIndex];
}
