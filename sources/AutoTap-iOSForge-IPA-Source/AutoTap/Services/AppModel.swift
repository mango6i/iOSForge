...
final class AppModel: ObservableObject {
    ...
    @Published var runDestination: RunDestination {
        didSet { UserDefaults.standard.set(runDestination.rawValue, forKey: "AutoTap.RunDestination") }
    }
    ...
    private init() {
        runDestination = RunDestination(
            rawValue: UserDefaults.standard.string(forKey: "AutoTap.RunDestination") ?? "system"
        ) ?? .system
    ...
    func startSelectedProfile() {
        ...
        if runDestination == .system {
            let trimmed = targetBundleIdentifier.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !trimmed.isEmpty else {
                bannerMessage = "请先填写目标 App 的 Bundle ID。"
                return
            }
        ...
        engine.start(profile: profile, destination: destination) {
            destination == .preview || ATTouchDispatcher.shared().openApplication(bundleIdentifier: bundleID)
        }
    }
    ...
    func scenePhaseChanged(_ phase: ScenePhase) {
        switch phase {
        case .background:
            if runDestination == .system && engine.isActive {
