...
final class AutomationEngine: ObservableObject {
    ...
    private func countdown(
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let screenSize = self.mappedScreenSize(base, orientation: profile.orientation)
    ...
    private func mappedScreenSize(_ size: CGSize, orientation: TargetOrientation) -> CGSize {
        switch orientation {
        case .portrait: return CGSize(width: min(size.width, size.height), height: max(size.width, size.height))
        case .landscape: return CGSize(width: max(size.width, size.height), height: min(size.width, size.height))
        }
    }
    ...
    private func finish(token: UUID, state: AutomationEngineState) {
        ...
        DispatchQueue.main.async {
            BackgroundKeeper.shared.stop()
            self.elapsedSeconds = self.startedAt.map { Date().timeIntervalSince($0) } ?? 0
            self.startedAt = nil
            self.currentActionID = nil
            self.state = state
            let message: String
