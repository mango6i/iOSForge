...
struct GeneralSettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section(header: Text("悬浮窗按钮透明度")) {
                HStack {
                    Spacer()
                    ControlBarPreview(scale: model.controlScale)
                        .opacity(model.controlOpacity)
                    Spacer()
                }
                Slider(value: $model.controlOpacity, in: 0.2...1.0)
                HStack { Text("更透明"); Spacer(); Text("适中"); Spacer(); Text("不透明") }
                    .font(.caption).foregroundColor(.secondary)
                Text("当前 \(Int((model.controlOpacity * 100).rounded()))%")
                    .font(.caption).foregroundColor(.secondary)
            }
