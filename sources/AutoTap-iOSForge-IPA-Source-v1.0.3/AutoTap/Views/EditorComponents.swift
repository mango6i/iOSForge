        ...
        GeometryReader { proxy in
            ...
            ZStack {
                ...
                ForEach(actions.indices, id: \.self) { index in
                    ...
                    let start = action.start.cgPoint(in: size)
                    let end = action.end.cgPoint(in: size)
                    if action.kind == .swipe {
                        SwipePath(start: start, end: end)
                        SwipeStartHandle(selected: selectedID == action.id, active: activeID == action.id)
                            .position(start)
                        ...
                        SwipeEndHandle(
                            ...
                            angle: Angle(radians: Double(atan2(end.y - start.y, end.x - start.x)))
                        )
                            .position(end)
...
private struct SwipeStartHandle: View {
    let selected: Bool
    let active: Bool
    ...
    var body: some View {
        Circle()
            ...
            .overlay(
                Circle().stroke(
                    active ? Color.green : AppTheme.warning,
                    lineWidth: selected || active ? 4 : 3
                )
            )
            .frame(width: active ? 32 : 27, height: active ? 32 : 27)
            .shadow(color: (active ? Color.green : AppTheme.warning).opacity(0.35), radius: active ? 10 : 4)
    }
}
