        ...
        ScrollView {
            VStack(spacing: 16) {
                ...
                if let index = selectedActionIndex {
                    ...
                    ActionInspector(profileID: profile.id, actions: profile.actions, index: index, category: category)
    ...
    private var profileHeader: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            TextField("脚本名称", text: Binding(
                get: { profile.name },
                set: { value in model.updateProfile(id: profile.id) { $0.name = value } }
            ))
            .font(.system(size: 20, weight: .semibold))

            if category.isRecording {
                NavigationLink(destination: RecordingPositionEditorView(category: category, store: store).environmentObject(model)) {
                    HStack {
                        Label(
                            category == .gestureRecording ? "按录制顺序执行动作" : "按编号顺序执行",
                            systemImage: category == .gestureRecording ? "hand.draw.fill" : "circle.grid.2x2.fill"
                        )
                            .font(.subheadline)
                            .foregroundColor(AppTheme.accent)
                        Spacer()
                        Text("\(profile.actions.count) 个\(category == .gestureRecording ? "动作" : "目标")")
                            .font(.caption)
    ...
    private var runSettings: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            IntegerEntryRow(title: "启动倒计时", suffix: "秒（0 为立即开始）", value: Binding(
                ...
                set: { value in model.updateProfile(id: profile.id) { $0.startDelaySeconds = min(max(value, 0), 15) } }
            ))
            IntegerEntryRow(title: "循环次数", suffix: "次（0 为持续循环）", value: Binding(
                get: { profile.cycleCount },
                set: { value in model.updateProfile(id: profile.id) { $0.cycleCount = min(max(value, 0), 999_999) } }
            ))
            ...
            if mode == .multiple {
                IntegerEntryRow(title: "每轮完成后间隔", suffix: "秒（0 为立即下一轮）", value: Binding(
                    ...
                    set: { value in model.updateProfile(id: profile.id) { $0.cycleIntervalSeconds = min(max(value, 0), 86_400) } }
                ))
            }
            if category.isRecording {
                Divider()
                Toggle("暂停后从编号 1 重新开始", isOn: Binding(
                    ...
                    set: { value in model.updateProfile(id: profile.id) { $0.restartRecordingFromBeginningOnResume = value } }
                ))
...
private struct RecordingPositionEditorView: View {
    ...
    var body: some View {
            ...
            VStack(spacing: 12) {
                HStack {
                    VStack(alignment: .leading, spacing: 3) {
                        Text(category == .gestureRecording ? "拖动起点或终点调整手势" : "拖动数字调整录制点位")
                            .font(.headline)
                        Text(category == .gestureRecording
                             ? "这里只修改保存的动作；手势回放时不会显示数字目标。"
                             : "这里只修改保存的坐标；回放悬浮层中的数字保持锁定。")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                    Spacer()
                ...
                ActionCanvas(
                    ...
                    onMove: { id, start, end in
                        model.moveAction(profileID: profile.id, id: id, start: start, end: end)
                    }
                )
                .aspectRatio(
        ...
        .background(Color(UIColor.systemGroupedBackground).ignoresSafeArea())
        .navigationBarTitle(category == .gestureRecording ? "录制动作" : "录制点位", displayMode: .inline)
        .onAppear {
...
private struct ActionInspector: View {
    ...
    private func intBinding(_ keyPath: WritableKeyPath<AutomationAction, Int>) -> Binding<Int> {
        Binding(
            ...
            set: { value in model.updateAction(profileID: profileID, id: action.id) { $0[keyPath: keyPath] = value } }
        )
    }
    ...
    private var intervalUnitBinding: Binding<IntervalUnit> {
        Binding(
            ...
            set: { value in model.updateAction(profileID: profileID, id: action.id) { $0.changeIntervalUnit(to: value) } }
        )
    }
    ...
    private var intervalValueBinding: Binding<Int> {
        Binding(
            ...
            set: { value in model.updateAction(profileID: profileID, id: action.id) { $0.setIntervalDisplayValue(value) } }
        )
    }
}
