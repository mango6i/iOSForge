...
struct ModeEditorView: View {
    ...
    private var profileHeader: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if category.isRecordingCategory {
...
private struct RecordingPositionEditorView: View {
    ...
    var body: some View {
        ...
        .background(Color(UIColor.systemGroupedBackground).ignoresSafeArea())
        .navigationBarTitle(category == .gestureRecording ? "手势路径" : "录制点位", displayMode: .inline)
        .onAppear {
...
private struct ActionInspector: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            // Multi-target and recorded scripts have no per-target continuous
            // execution setting; they run each numbered target once per round.
            if category == .single {
