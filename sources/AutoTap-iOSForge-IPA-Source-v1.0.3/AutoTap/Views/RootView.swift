...
private struct LandingView: View {
        ...
        .sheet(isPresented: $showModeEditor, onDismiss: {
            ...
            NavigationView {
                ModeEditorView(
                    ...
                    store: store,
                    engine: model.engine(for: editorCategory)
                )
                    .environmentObject(model)
            }
            .navigationViewStyle(StackNavigationViewStyle())
    ...
    private func modeButtons(
        ...
        let modeEngine = model.engine(for: category)
        return HStack(spacing: layout.ultraCompact ? 4 : 8) {
            NavigationLink(destination: ModeEditorView(mode: mode, category: category, store: store, engine: modeEngine).environmentObject(model)) {
                compactActionLabel(title: "设置", icon: "gearshape.fill", layout: layout)
            }
            .disabled(isBlocked || modeEngine.isRunning)
            .opacity(isBlocked || modeEngine.isRunning ? 0.45 : 1)
            ...
            // Never disable the mode switch itself. After closing one mode,
            // SwiftUI may need one render pass to clear `isBlocked`; disabling
            // this button made a fast tap on the next mode disappear. The model
            // owns the atomic close-and-switch operation, so the last tap wins.
        }
    }
    ...
    private func recordingCard(captureMode: RecordingCaptureMode, layout: LandingLayout) -> some View {
        ...
        let subtitle = model.isRecordingActive
            ...
            ? model.recordingDiagnosticsText
            : (isOpen
        ...
        .onReceive(Timer.publish(every: 0.4, on: .main, in: .common).autoconnect()) { _ in
            // Diagnostics build: keep the capture-path numbers live on the card.
        }
    }
    ...
    private func recordingButtons(
        ...
        let modeEngine = model.engine(for: category)
        return HStack(spacing: layout.ultraCompact ? 4 : 8) {
            NavigationLink(destination: RecordingScriptsView(category: category, store: store).environmentObject(model)) {
                compactActionLabel(title: "脚本 \(savedCount)", icon: "gearshape.fill", layout: layout)
            }
            .disabled(isBlocked || model.isRecordingActive || modeEngine.isRunning)
            .opacity(isBlocked || model.isRecordingActive || modeEngine.isRunning ? 0.45 : 1)

            Button(action: {
                if captureMode == .gestures { _ = model.toggleGestureRecordingMode() }
                else { _ = model.toggleTapRecordingMode() }
            }) {
                compactPrimaryLabel(
                    title: isOpen ? "关闭" : "开启",
            ...
            // Keep switching enabled for the same reason as point modes. The
            // action consults live AppModel state rather than this view's
            // possibly stale `isOpen` snapshot.
        }
    }
    ...
    private func statusCard(layout: LandingLayout) -> some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                if !layout.ultraCompact {
                    Text("已执行 \(model.activeEngine.completedActions) 次 · \(Int(model.activeEngine.elapsedSeconds)) 秒")
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .lineLimit(1)
                }
            ...
            Spacer()
            if model.activeEngine.isActive {
                Button("停止") { model.stop() }
                    .foregroundColor(.red)
            } else if model.activeEngine.completedActions > 0 || model.activeEngine.elapsedSeconds > 0 {
                Button("清空") { model.activeEngine.clearRunHistory() }
                    .foregroundColor(AppTheme.accent)
            }
        }
        .frame(maxWidth: .infinity)
    ...
    private var statusTitle: String {
        switch model.activeEngine.state {
    ...
    private func start(mode: AutomationMode) {
        if mode == .single { _ = model.openSinglePointMode() }
        else { _ = model.openMultiPointMode() }
    }
...
private struct LandingLayout {
    ...
    init(size: CGSize) {
        ...
        actionVerticalPadding = useUltraCompactLayout ? 8 : (useCompactLayout ? 10 : 12)
        ...
        .sheet(isPresented: $showEditor, onDismiss: {
            ...
            NavigationView {
                ModeEditorView(
                    ...
                    category: category,
                    store: store,
                    engine: model.engine(for: category)
                )
                    .environmentObject(model)
            }
            .navigationViewStyle(StackNavigationViewStyle())
    ...
    private func scriptCard(_ script: AutomationProfile) -> some View {
        VStack(alignment: .leading, spacing: 13) {
            HStack(alignment: .top) {
                ...
                Spacer()
                if store.selectedProfile(for: category)?.id == script.id {
