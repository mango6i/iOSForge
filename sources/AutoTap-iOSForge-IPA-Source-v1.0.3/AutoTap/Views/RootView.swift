        ...
        .alert(isPresented: bannerBinding) {
            if let update = model.availableUpdate {
                return Alert(
                    ...
                    message: Text(update.notes.isEmpty
                        ? "新版本已经发布。点击“前往下载”获取最新成品。"
                        : "主要更新：\n\(update.notes)"),
...
private struct LandingView: View {
                ...
                VStack(spacing: layout.spacing) {
                    ...
                    VStack(spacing: layout.spacing) {
                        ...
                        recordingCard(
                            ...
                            gradient: tapRecordingGradient,
                            layout: layout
                        )
                        recordingCard(
                            captureMode: .gestures,
                            title: "手势录制",
                            subtitle: "混合录制点击与滑动",
                            icon: "hand.draw.fill",
                            gradient: gestureRecordingGradient,
                            layout: layout
                        )
                    }
                    ...
                    Button(action: { model.checkForUpdates() }) {
                        HStack(spacing: 5) {
                            ...
                            Text(model.isCheckingForUpdates
                                 ? "正在检查更新…"
                                 : "AutoTap \(model.displayVersion) · iOS 15+")
                                .lineLimit(1)
                        }
    ...
    private func modeCard(
        ...
        .frame(
            height: max(1, layout.modeCardHeight - layout.cardPadding * 2),
            alignment: .center
        )
    ...
    private func recordingCard(
        ...
        let isOpen = model.isRecording && model.recordingCaptureMode == captureMode
        let isBlocked = (model.activeOverlayMode != nil && !isOpen) || (model.isRecording && !isOpen)
        let savedCount = store.profiles.filter {
            $0.category == captureMode.category && !$0.actions.isEmpty
        }.count
        let liveSubtitle: String = {
            if model.isRecordingActive && isOpen {
                return captureMode == .taps
                    ? "正在录制 · 已记录 \(model.recordingCount) 个点击"
                    : "正在录制 · 已记录 \(model.recordingCount) 个动作"
            }
            return isOpen ? "悬浮条已开启，点录制键开始" : subtitle
        }()
        ...
        return VStack(alignment: .leading, spacing: layout.cardSpacing) {
            ...
            HStack(spacing: 8) {
                NavigationLink(
                    destination: RecordingScriptsView(
                        category: captureMode.category,
                        captureMode: captureMode,
                        store: store,
                        engine: engine
                    ).environmentObject(model)
                ) {
                    compactActionLabel(title: "脚本 \(savedCount)", icon: "folder.fill", layout: layout)
                }
    ...
    private var tapRecordingGradient: LinearGradient {
        LinearGradient(
    ...
    private var gestureRecordingGradient: LinearGradient {
        LinearGradient(
    ...
    private func generalSettingsCard(layout: LandingLayout) -> some View {
        NavigationLink(destination: GeneralSettingsView().environmentObject(model)) {
            ...
            .frame(maxWidth: .infinity)
            .frame(
                height: max(1, layout.secondaryCardHeight - layout.secondaryCardPadding * 2),
                alignment: .center
            )
    ...
    private func statusCard(layout: LandingLayout) -> some View {
        HStack(spacing: layout.compact ? 7 : 12) {
            Circle()
                .fill(statusColor)
                .frame(width: layout.compact ? 9 : 11, height: layout.compact ? 9 : 11)
            VStack(alignment: .leading, spacing: 2) {
                Text(statusTitle)
                    .font(.system(size: layout.cardTitleSize, weight: .bold))
                    .lineLimit(1)
...
private struct LandingLayout {
    ...
    init(size: CGSize) {
        ...
        let isVeryShort = size.height < 500
        let useCompactLayout = size.height < 780 || (isLandscape && size.height < 620)
        let hideDetailText = size.height < 680 || isVeryShort
        let horizontalPadding: CGFloat = isVeryShort ? 6 : (useCompactLayout ? 8 : 14)

        let usesWideColumns = size.width >= 700 || (size.width >= 540 && isLandscape)
        usesColumns = usesWideColumns
        compact = useCompactLayout
        hidesDetails = hideDetailText
        contentWidth = min(1040, max(1, size.width - horizontalPadding * 2))
        verticalPadding = isVeryShort ? 2 : (useCompactLayout ? 5 : 9)
        spacing = isVeryShort ? 3 : (useCompactLayout ? 5 : 8)
        footerHeight = min(max(size.height * 0.032, 14), 27)
        heroHeight = min(max(size.height * 0.06, 22), 54)
        secondaryCardHeight = min(max(size.height * 0.076, 28), 68)
        // Exact height budget: four mode cards, the original settings/status
        // section, the two original footer rows and every gap share the full
        // GeometryReader height. This keeps the established order while
        // filling—not overflowing—every iPhone/iPad resolution.
        let totalGapCount: CGFloat = usesWideColumns ? 7 : 8
        let settingsHeight = usesWideColumns
            ? secondaryCardHeight
            : secondaryCardHeight * 2 + spacing
        let fixedHeight = verticalPadding * 2
            + spacing * totalGapCount
            + heroHeight
            + settingsHeight
            + footerHeight * 2
        modeCardHeight = max(46, (size.height - fixedHeight) / 4)
        heroIconSize = isVeryShort ? 22 : (hideDetailText ? 32 : (useCompactLayout ? 38 : 46))
        heroTitleSize = isVeryShort ? 18 : (hideDetailText ? 22 : (useCompactLayout ? 25 : 29))
        modeIconSize = isVeryShort ? 22 : (hideDetailText ? 30 : (useCompactLayout ? 35 : 42))
        smallIconSize = isVeryShort ? 22 : (hideDetailText ? 29 : (useCompactLayout ? 34 : 40))
        cardTitleSize = isVeryShort ? 12 : (hideDetailText ? 14 : (useCompactLayout ? 16 : 18))
        cardPadding = isVeryShort ? 3 : (hideDetailText ? 6 : (useCompactLayout ? 8 : 11))
        secondaryCardPadding = isVeryShort ? 3 : (hideDetailText ? 6 : (useCompactLayout ? 8 : 10))
        cardCornerRadius = isVeryShort ? 10 : (useCompactLayout ? 15 : 18)
        cardSpacing = isVeryShort ? 2 : (hideDetailText ? 4 : (useCompactLayout ? 6 : 8))
        actionFontSize = isVeryShort ? 10 : (hideDetailText ? 11 : (useCompactLayout ? 13 : 15))
        actionVerticalPadding = isVeryShort ? 2 : (hideDetailText ? 5 : (useCompactLayout ? 6 : 8))
    }
}
...
private struct RecordingScriptsView: View {
    ...
    var body: some View {
                ...
                if scripts.isEmpty {
                    VStack(spacing: 12) {
                        Image(systemName: "tray")
                            .font(.system(size: 34))
                            .foregroundColor(.secondary)
                        Text("还没有保存的\(captureMode.title)脚本")
                            .font(.headline)
                        Text("返回首页开启\(captureMode.title)，点悬浮条录制键开始；结束并命名后会显示在这里。")
                            .font(.caption)
                            .foregroundColor(.secondary)
        ...
        .navigationBarTitle("\(captureMode.title)脚本", displayMode: .inline)
            ...
            ProfileDocumentPicker(
                ...
                onPick: { url in
                    ...
                    do {
                        ...
                        model.bannerMessage = "\(captureMode.title)脚本已导入。"
                    } catch {
        ...
        VStack(alignment: .leading, spacing: 13) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(script.name.isEmpty ? "未命名\(captureMode.title)脚本" : script.name)
                        .font(.headline)
                    Text(script.cycleCount == 0
                         ? "\(script.actions.count) 个动作 · 持续循环"
                         : "\(script.actions.count) 个动作 · 循环 \(script.cycleCount) 次")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
                Spacer()
    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 16) {
                HStack(spacing: 10) {
                    ...
                    VStack(alignment: .center, spacing: 2) {
                        Text("录制完成")
                            ...
                            .multilineTextAlignment(.center)
                        Text(model.recordingCaptureMode == .taps
                             ? "共记录 \(model.recordingCount) 个点击"
                             : "共记录 \(model.recordingCount) 个动作")
                            .font(.caption)
                            .foregroundColor(.secondary)
                            .multilineTextAlignment(.center)
                    }
                ...
                Text("保存后会显示在\(model.recordingCaptureMode.title)的独立脚本列表，可修改每个动作的间隔、时长和整体循环次数。")
                    .font(.caption)
                    .foregroundColor(.secondary)
                    .multilineTextAlignment(.center)
