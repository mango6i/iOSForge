        ...
        GeometryReader { geometry in
            ...
            ZStack {
                ...
                VStack(spacing: layout.spacing) {
                    ...
                    LazyVGrid(columns: layout.modeColumns, spacing: layout.spacing) {
                        modeCard(
                            mode: .single,
                            title: "单点模式",
...
private struct LandingLayout {
    ...
    init(size: CGSize) {
        ...
        let hideDetailText = size.height < 680 || (isLandscape && size.height < 500)
        let horizontalPadding: CGFloat = useCompactLayout ? 8 : 14

        modeColumnCount = size.width >= 760 ? 4 : 2
        compact = useCompactLayout
        hidesDetails = hideDetailText
        contentWidth = min(1040, max(1, size.width - horizontalPadding * 2))
        verticalPadding = useCompactLayout ? 5 : 9
        spacing = useCompactLayout ? 5 : 8
        heroHeight = hideDetailText ? 34 : (useCompactLayout ? 42 : 50)
        heroIconSize = hideDetailText ? 32 : (useCompactLayout ? 38 : 46)
        heroTitleSize = hideDetailText ? 22 : (useCompactLayout ? 25 : 29)
        modeIconSize = hideDetailText ? 30 : (useCompactLayout ? 35 : 42)
        smallIconSize = hideDetailText ? 29 : (useCompactLayout ? 34 : 40)
        cardTitleSize = hideDetailText ? 14 : (useCompactLayout ? 16 : 18)
        cardPadding = hideDetailText ? 6 : (useCompactLayout ? 8 : 11)
        secondaryCardPadding = hideDetailText ? 6 : (useCompactLayout ? 8 : 10)
        cardCornerRadius = useCompactLayout ? 15 : 18
        cardSpacing = hideDetailText ? 4 : (useCompactLayout ? 6 : 8)
        actionFontSize = hideDetailText ? 11 : (useCompactLayout ? 13 : 15)
        actionVerticalPadding = hideDetailText ? 5 : (useCompactLayout ? 6 : 8)
    }
}
