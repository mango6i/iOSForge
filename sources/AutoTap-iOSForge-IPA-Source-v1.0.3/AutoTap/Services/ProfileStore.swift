...
final class ProfileStore: ObservableObject {
    ...
    func addRecordedProfile(
        ...
        precondition(category.isRecordingCategory)
        let profile = AutomationProfile(
            name: name,
            mode: .multiple,
            profileCategory: category,
    ...
    func duplicate(_ id: UUID) -> UUID? {
        ...
        copy.createdAt = Date()
        copy.updatedAt = Date()
        let supportsContinuousExecution = copy.category == .single
        copy.actions = copy.actions.map {
            var action = $0
            action.id = UUID()
            if !supportsContinuousExecution { action.repeatCount = 1 }
            return action
        }
    ...
    func canDelete(_ id: UUID) -> Bool {
        ...
        if profile.category.isRecordingCategory { return true }
    ...
    func delete(_ id: UUID) {
        ...
        if !deletedCategory.isRecordingCategory && profiles(for: deletedCategory).isEmpty {
    ...
    func exportURL(for id: UUID) throws -> URL {
        guard var profile = profile(id: id) else { throw StoreError.profileNotFound }
        if profile.category != .single {
            profile.actions = profile.actions.map { action in
                var normalized = action
                normalized.repeatCount = 1
                return normalized
            }
    ...
    func importProfile(from url: URL, category: AutomationProfileCategory) throws -> UUID {
        ...
        profile.actions = profile.actions.map {
            var action = $0
            action.id = UUID()
            if category != .single { action.repeatCount = 1 }
            return action
        }
        let previousSelection = selectedProfileID
        profiles.append(profile)
    ...
    private func load() {
        ...
        if var saved = savedProfiles {
            ...
            for index in saved.indices {
                ...
                if saved[index].category != .single,
                   saved[index].actions.contains(where: { $0.repeatCount != 1 }) {
                    saved[index].actions = saved[index].actions.map { action in
                        var normalized = action
                        normalized.repeatCount = 1
                        return normalized
                    }
