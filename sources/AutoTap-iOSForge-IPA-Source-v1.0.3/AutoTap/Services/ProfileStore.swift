...
final class ProfileStore: ObservableObject {
    ...
    @discardableResult
    func addRecordedProfile(
        name: String,
        actions: [AutomationAction],
        category: AutomationProfileCategory
    ) throws -> UUID {
        guard category.isRecording else { throw StoreError.invalidRecordingCategory }
        let profile = AutomationProfile(
            name: name,
            mode: .multiple,
            profileCategory: category,
    ...
    func suggestedRecordingName(for category: AutomationProfileCategory) -> String {
        let prefix = category == .gestureRecording ? "新建手势脚本" : "新建脚本"
        let number = nextAvailableNumber(prefix: "\(prefix) ", category: category)
    ...
    func canDelete(_ id: UUID) -> Bool {
        ...
        if profile.category.isRecording { return true }
        if profiles(for: profile.category).count > 1 { return true }
        let isUntouchedPlaceholder = profile.actions.isEmpty
            && profile.name.hasPrefix("新建\(profile.category.title)")
            && !profile.name.contains("（导入）")
        return !isUntouchedPlaceholder
    }

    func delete(_ id: UUID) {
        guard let deletedProfile = profiles.first(where: { $0.id == id }),
              canDelete(id) else { return }
        let deletedCategory = deletedProfile.category
        profiles.removeAll(where: { $0.id == id })
        if !deletedCategory.isRecording && profiles(for: deletedCategory).isEmpty {
            let replacement = AutomationProfile(
                name: "新建\(deletedCategory.title) 1",
                mode: deletedCategory.mode,
                profileCategory: deletedCategory
            )
            profiles.append(replacement)
        }
        if profiles.isEmpty { profiles = [.starter] }
        if selectedProfileID == id {
            selectedProfileID = profiles.first(where: { $0.category == deletedCategory })?.id
                ?? profiles.first?.id
        }
        if UserDefaults.standard.string(forKey: categorySelectionKey(deletedCategory)) == id.uuidString {
    ...
    private func load() {
        ...
        if var saved = savedProfiles {
            ...
            for index in saved.indices where saved[index].profileCategory == nil {
                didMigrate = true
                if saved[index].name.hasPrefix("录制脚本") || saved[index].name.hasPrefix("新建脚本") {
            ...
            for index in saved.indices {
                ...
                // The old default start delay was 3 seconds; every saved script
                // now starts after 1 second unless the user picked another value
                // on purpose.
                if saved[index].startDelaySeconds == 3 {
                    saved[index].startDelaySeconds = 1
                    didMigrate = true
                }
            }
            profiles = saved
    ...
    private func nextAvailableNumber(prefix: String, category: AutomationProfileCategory) -> Int {
        let occupied = Set(profiles(for: category).compactMap { profile -> Int? in
            guard profile.name.hasPrefix(prefix) else { return nil }
            let suffix = profile.name.dropFirst(prefix.count)
                .trimmingCharacters(in: .whitespacesAndNewlines)
            return Int(suffix)
        })
        var candidate = profiles(for: category).count + 1
