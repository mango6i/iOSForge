...
final class AutomationEngine: ObservableObject {
    ...
    private func perform(
        ...
        case .swipe:
            // Give iOS one display frame to accept the DOWN contact before
            // sending MOVE frames. Sending both back-to-back was interpreted
            // as a stationary tap on some devices and recorded swipes appeared
            // to do nothing during playback.
            let swipeDuration = max(0.08, TimeInterval(action.durationMilliseconds) / 1000)
            let steps = min(max(Int(ceil(swipeDuration * 60)), 5), 120)
            let firstFrameDelay = min(0.02, swipeDuration / Double(steps))
            queue.asyncAfter(deadline: .now() + firstFrameDelay) { [weak self] in
