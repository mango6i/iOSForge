...
final class AutomationEngine: ObservableObject {
    ...
    private func countdown(
        ...
        if ATSystemOverlayIsDeviceLocked() {
            // The native lock gate closes before UIKit receives the lock
            // notification. Preserve the countdown until AppModel marks the
            // engine paused instead of launching against the lock screen.
    ...
    private func runStep(
        ...
        if ATSystemOverlayIsDeviceLocked() {
            // The native lock gate closes before UIKit receives the lock
            // notification. Preserve this exact step until AppModel marks the
            // engine paused instead of treating a deliberately blocked HID
            // frame as a script failure.
            queue.asyncAfter(deadline: .now() + 0.25) { [weak self] in
                self?.runStep(
                    token: token,
                    profile: profile,
