...
final class AppModel: ObservableObject {
    ...
    /// Opacity of the floating control bar, adjustable from 通用设置.
    @Published var controlOpacity: Double {
        didSet {
            UserDefaults.standard.set(controlOpacity, forKey: "AutoTap.ControlOpacity")
    ...
    private init() {
        ...
        let savedControlOpacity = UserDefaults.standard.double(forKey: "AutoTap.ControlOpacity")
        controlOpacity = savedControlOpacity == 0 ? 1 : min(max(savedControlOpacity, 0.2), 1.0)
        ...
        // screen locked — devices can emit it while AutoTap is simply
        // backgrounded. Without this counterpart the global lock gate stayed
        // closed afterwards and the native HID callback dropped every event.
    ...
    func refreshSystemOverlay() {
        ...
        synchronizeSystemOverlayRole()
        // Every mode owns its own overlay controller, so publish the opacity to
        // all of them — the floating bar of whichever mode is opened next
        // already carries the user's choice.
        for overlay in allOverlayControllers { overlay.controlOpacity = CGFloat(controlOpacity) }
