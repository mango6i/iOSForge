...
final class AppModel: ObservableObject {
    ...
    private func resetOverlayForNewMode() {
        systemOverlay.setEditingSuppressed(false)
        systemOverlay.setGesturePaths([], durations: [])
        systemOverlay.setRecordingEnabled(false)
        systemOverlay.setRecordingActive(false)
        systemOverlay.setRecordingCapturesGestures(false)
        if systemOverlay.isVisible { systemOverlay.hide() }
    ...
    private func showSystemOverlay(mode: AutomationMode, category: AutomationProfileCategory) -> Bool {
        ...
        // Opening a different mode: drop the previous overlay completely so no
        // marker, suppression flag or recorder state survives the switch.
        resetOverlayForNewMode()
    ...
    func startRecording(captureMode: RecordingCaptureMode) -> Bool {
        ...
        systemOverlay.setRecordingActive(false)
        systemOverlay.setRecordingCapturesGestures(captureMode == .gestures)
    ...
    func finishRecordingCapture() {
        ...
        guard let bundleID = Bundle.main.bundleIdentifier,
            ...
            systemOverlay.setRecordingEnabled(true)
            systemOverlay.setRecordingActive(true)
            systemOverlay.setRecordingCapturesGestures(recordingCaptureMode == .gestures)
    ...
    private func configureSystemOverlay() {
        ...
        systemOverlay.recordSwipeHandler = { [weak self] startX, startY, endX, endY, intervalMilliseconds, durationMilliseconds in
            let append: () -> Void = { [weak self] in
                self?.appendRecordedAction(
                    kind: .swipe,
                    normalizedX: startX,
        ...
        systemOverlay.recordGestureHandler = { [weak self] points, offsets, intervalMilliseconds in
            let append: () -> Void = { [weak self] in
