...
final class AppModel: ObservableObject {
    ...
    private func replaceOverlaySession(with session: OverlaySession) {
        ...
        systemOverlay.configureRecording(enabled: false, capturesGestures: false, active: false)
    ...
    private func configureSystemOverlay() {
        ...
        systemOverlay.closeHandler = { [weak self] in
            ...
            DispatchQueue.main.async {
                guard generation == self.overlaySessionGeneration else { return }
    ...
    /// The Objective-C HUD is intentionally reused across modes so its
    /// WindowServer context remains stable. Reuse is safe only when its role is
    /// explicitly reassigned on every show/refresh; otherwise a recorder bar
    /// and its old markers can leak into single- or multi-point mode.
