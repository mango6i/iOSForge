...
enum RecordingCaptureMode: Equatable {
    case taps
    case gestures

    var title: String { self == .taps ? "点击录制" : "手势录制" }

    var category: AutomationProfileCategory {
        self == .taps ? .recording : .gestureRecording
    }
}
...
final class AppModel: ObservableObject {
    ...
    @Published private(set) var recordingCaptureMode: RecordingCaptureMode = .taps
    ...
    private var nextRecordingDefaultName: String {
        let existingCount = store.profiles.filter {
            $0.category == recordingCaptureMode.category && !$0.actions.isEmpty
        }.count
        return "新建脚本\(existingCount + 1)"
    }
    ...
    func openRecordedScript(_ id: UUID) -> Bool {
        ...
        guard let profile = store.profile(id: id), profile.category.isRecordingCategory else { return false }
        if systemOverlay.isVisible { closeSystemOverlay() }
        selectProfile(id)
        return showSystemOverlay(mode: .multiple, category: profile.category)
    }
    ...
    func closeSystemOverlay() {
        ...
        systemOverlay.setGestureRecordingEnabled(false)
    ...
    func startRecording(captureMode: RecordingCaptureMode) -> Bool {
        ...
        recordedActions.removeAll()
        recordingCount = 0
        recordingNameDraft = nextRecordingDefaultName
        isNamingRecording = false
        pendingRecordingNameActivation = false
        isRecording = true
        isRecordingActive = false
        activeOverlayMode = .multiple
        activeProfileCategory = captureMode.category
        activeOverlayProfileID = nil
        selectedActionID = nil
        systemOverlay.setGestureRecordingEnabled(captureMode == .gestures)
    ...
    func saveRecordedScript() {
        ...
        let finalName = name.isEmpty ? nextRecordingDefaultName : name
        let id: UUID
        do {
            id = try store.addRecordedProfile(
                name: finalName,
                actions: recordedActions,
                category: recordingCaptureMode.category
            )
        } catch {
        ...
        systemOverlay.hide()
        BackgroundKeeper.shared.stop()
        bannerMessage = "\(recordingCaptureMode.title)脚本“\(finalName)”已保存。"
    }
    ...
    private func configureSystemOverlay() {
        ...
        systemOverlay.moveHandler = { [weak self] index, x, y in
            DispatchQueue.main.async {
                ...
                let start = NormalizedPoint(x: Double(x), y: Double(y))
                let end: NormalizedPoint?
                if action.kind == .swipe {
                    end = NormalizedPoint(
                        x: start.x + action.end.x - action.start.x,
                        y: start.y + action.end.y - action.start.y
                    )
                } else {
                    end = nil
                }
    ...
    private func overlaySnapshot() -> (points: [NSValue], actionSettings: [[String: NSNumber]], multiple: Bool, selectedIndex: Int, activeIndex: Int, editingEnabled: Bool, countdownSeconds: Int) {
        let usesUnsavedRecording = activeProfileCategory?.isRecordingCategory == true && activeOverlayProfileID == nil
        ...
        let actionSettings: [[String: NSNumber]] = actions.map { action in
            var settings: [String: NSNumber] = [
                "kind": NSNumber(value: action.kind == .swipe ? 1 : 0),
                "endX": NSNumber(value: action.end.x),
                "endY": NSNumber(value: action.end.y),
        ...
        let editingEnabled = activeProfileCategory?.isRecordingCategory != true
