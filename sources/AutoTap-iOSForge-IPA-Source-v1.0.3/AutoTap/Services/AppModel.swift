...
final class AppModel: ObservableObject {
    ...
    /// Temporary diagnostics line shown on the recording card while capturing.
    /// Reads the controller that actually owns the capture hook, so a second
    /// (stale) HUD instance can never hide the real numbers again.
    var recordingDiagnosticsText: String {
        guard isRecording else { return "" }
        return "Swift\(recordingCount) " + ATSystemOverlayController.activeRecorderDiagnostics()
    }
    ...
    private func setActiveRecorder(_ active: Bool) {
        ...
        // The capture gate lives in the native controller. Pushing it here is
        // what actually starts sampling; without this the HUD stayed disabled
        // until some later refresh happened to sync it, which is why a take
        // sometimes recorded nothing at all (count stuck at 0).
    ...
    private init() {
        ...
        NotificationCenter.default.publisher(for: UIApplication.protectedDataWillBecomeUnavailableNotification)
            ...
            .store(in: &cancellables)
        // "Protected data will become unavailable" does not always mean the
        // screen actually locked — devices can emit it while AutoTap is simply
        // backgrounded. Without this counterpart the global lock gate stayed
        // closed, and the native HID callback dropped every event afterwards:
        // recording then stopped at a random count and the overlay stopped
        // refreshing, even though nothing was wrong on screen.
        NotificationCenter.default.publisher(for: UIApplication.protectedDataDidBecomeAvailableNotification)
            .sink { [weak self] _ in
                DispatchQueue.main.async { self?.resumeAfterDeviceUnlockIfNeeded() }
            }
            .store(in: &cancellables)
    ...
    func startRecording(captureMode: RecordingCaptureMode) -> Bool {
        // A leftover running engine kept `running` alive on the HUD, which drew
        // the recorded numbers at 20% opacity — invisible on most backgrounds.
        // A recording session never runs an engine at the same time.
        for category in AutomationProfileCategory.allCases {
            engine(for: category).stop()
        }
    ...
    private func appendRecordedAction(
        ...
        // Foreground captures sample through the UIKit sendEvent hook, so a
        // refresh here cannot compete with HID delivery any more — and it is
        // the only way the numbers are guaranteed to be drawn. Background
        // captures keep the synchronous native marker only.
        if UIApplication.shared.applicationState == .active {
            refreshSystemOverlay()
        }
        // The native HUD appends tap markers synchronously on physical UP.
        // Do not rebuild or flush the complete hosted overlay for every contact:
        // when AutoTap itself is foreground, that work competes with delivery of
        // the next DOWN/UP pair. The draft remains authoritative for saving and
        // the native recorder already appended the matching visible number.
    }
    ...
    private func appendRecordedGesture(
        ...
        recordedActions.append(action)
        // Gesture capture also stays off the full overlay-refresh path; its
        // recorded points are shown later by the editor/playback renderer.
    }
    ...
    func refreshSystemOverlay() {
        // Never mutate or flush the SpringBoard-hosted layer tree during a
        // lock transition. The native gate already stopped input; the active
        // scene performs one full refresh after unlock.
        guard !ATSystemOverlayIsDeviceLocked(),
              activeOverlayMode != nil,
              systemOverlay.isVisible else { return }
        synchronizeSystemOverlayRole()
    ...
    private func configureSystemOverlay(_ overlay: ATSystemOverlayController, module: OverlayModuleID) {
        ...
        overlay.recordTapHandler = { [weak self] x, y, intervalMilliseconds, durationMilliseconds in
            guard let self, self.isActiveOverlayModule(module) else { return }
            // Captured samples must not be dropped by the session generation
            // counter. That counter is bumped by overlay rebuilds that do not
            // end the take (scene changes, session rebind), and gating on it
            // silently discarded every tap after the first one or two, leaving
            // the count stuck at 0/1/2/3. The active-module check below already
            // tells us whether this recorder still owns the sample.
    ...
    private func overlaySnapshot() -> (points: [NSValue], actionSettings: [[String: NSNumber]], multiple: Bool, selectedIndex: Int, activeIndex: Int, editingEnabled: Bool, countdownSeconds: Int, gesturePaths: [[NSValue]], gestureDurations: [NSNumber]) {
        // While a recording session is open the draft is the only source of
        // truth for the overlay points. Gating this on `activeOverlayProfileID
        // == nil` let a stale id (or any path that touched the selection) swap
        // the snapshot to an empty saved script, which zeroed the on-screen
        // numbers mid-take even though sampling had captured everything.
        let usesUnsavedRecording = activeProfileCategory?.isRecording == true
    ...
    private func pauseForDeviceLock() {
        ...
        // Normal close now tears down each module's HID client. Repeat this for
        // every controller during lock as an idempotent safety sweep, including
        // a controller whose show/registration was interrupted midway.
        ...
        // All mode engines intentionally share one process-wide IOHID transport.
        // The Darwin callback has already released it; this idempotent call also
        // covers protected-data notifications that did not pass that callback.
        ATTouchDispatcher.shared().suspendForDeviceLock()
        // Do not keep a privileged HID-capable process artificially alive on
        // the lock screen. A visible overlay reacquires its lease only after
        // AutoTap is active again; the task itself remains paused.
        BackgroundKeeper.shared.releaseAll()
        guard wasRunning || wasRecording || hadVisibleOverlay else { return }
        // Keep the hosted HUD session intact, but do not refresh/flush its
        // WindowServer layer tree while the lock screen is being installed.
        // Unlocking recreates input and the active scene refreshes it once.
        setScreenAwake(false)
    ...
    private func resumeAfterDeviceUnlockIfNeeded() {
        ...
        // Every controller was suspended when the device locked, so every one
        // of them has to be released here. Resuming only the controller that
        // happened to be visible left the others suppressed forever — that is
        // what froze the interface after a natural screen lock. The flag below
        // is only informational; it must not gate the restore.
        overlaySuspendedByDeviceLock = false
        var restoreFailure: String?
        for overlay in allOverlayControllers {
            if !overlay.resumeAfterDeviceUnlock() {
                restoreFailure = restoreFailure ?? overlay.diagnosticText
            }
        }
        if let restoreFailure {
            bannerMessage = restoreFailure
        } else if activeOverlayMode != nil {
            do {
                try activeEngine.keepBackgroundAlive()
            } catch {
                bannerMessage = error.localizedDescription
