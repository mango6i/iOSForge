...
@property (nonatomic, copy, nullable) void (^recordGestureHandler)(CGFloat startX, CGFloat startY, CGFloat endX, CGFloat endY, NSInteger intervalMilliseconds, NSInteger durationMilliseconds);
...
- (BOOL)showWithPoints:(NSArray<NSValue *> *)points
       ...
       countdownSeconds:(NSInteger)countdownSeconds NS_SWIFT_NAME(show(points:actionSettings:multiple:selectedIndex:activeIndex:markerScale:controlScale:editingEnabled:running:countdownSeconds:));
...
- (void)updateWithPoints:(NSArray<NSValue *> *)points
          actionSettings:(NSArray<NSDictionary<NSString *, NSNumber *> *> *)actionSettings
           ...
           editingEnabled:(BOOL)editingEnabled
                  running:(BOOL)running
         countdownSeconds:(NSInteger)countdownSeconds NS_SWIFT_NAME(update(points:actionSettings:multiple:selectedIndex:activeIndex:markerScale:controlScale:editingEnabled:running:countdownSeconds:));
...
/// Chooses whether a recording session should preserve drag/swipe paths.
/// When disabled, captured contacts are stored as ordinary numbered taps.
- (void)setGestureRecordingEnabled:(BOOL)enabled NS_SWIFT_NAME(setGestureRecordingEnabled(_:));
