...
@property (nonatomic, copy, nullable) void (^saveActionHandler)(NSInteger index, NSInteger intervalValue, NSInteger unitIndex, NSInteger millisecondsPreset, NSInteger secondsPreset, NSInteger minutesPreset, NSInteger durationMilliseconds, NSInteger repeatCount);
...
/// Temporary diagnostics build: one-line state of the capture path.
- (NSString *)recordingDiagnostics NS_SWIFT_NAME(recordingDiagnostics());

/// Diagnostics of whichever controller currently owns the capture hook, or a
/// "not registered" marker when sampling cannot run at all.
+ (NSString *)activeRecorderDiagnostics NS_SWIFT_NAME(activeRecorderDiagnostics());
