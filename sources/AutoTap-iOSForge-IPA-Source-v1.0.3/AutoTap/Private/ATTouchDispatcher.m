...
- (BOOL)sendAtX:(CGFloat)x y:(CGFloat)y phase:(ATTouchPhase)phase {
    ...
    // Match the digitizer phase masks used by iOS' own WebKit test harness:
    // DOWN announces contact, MOVE announces position, and UP announces both
    // the final position and release. Marking DOWN as a move as well caused
    // several gesture recognizers to keep treating the stream as a tap.
