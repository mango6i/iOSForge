...
- (BOOL)prepareForDispatch {
    // v1.0.2 resolves the UIWindow context before taking the dispatcher lock.
    // Do not move the whole HID setup onto the main queue: SpringBoard lock and
    // background callbacks also run there and must never wait for HID dispatch.
...
- (BOOL)sendAtX:(CGFloat)x y:(CGFloat)y phase:(ATTouchPhase)phase {
    // Keep the v1.0.2 lock ordering. Resolve UIKit state before entering the
    // dispatcher lock; pause/cancel can then release a finger during a system
    // background or lock transition without a main-queue/HID lock inversion.
