...
- (BOOL)prepareForDispatch {
    if (!NSThread.isMainThread) {
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{ prepared = [self prepareForDispatch]; });
        return prepared;
    }
    @synchronized (self) {
        ...
        // XXTouch keeps one IOHIDEventSystemClient alive for the process
        // lifetime. Releasing and recreating it on every start/resume works
        // once on some TrollStore systems, then silently stops routing events.
        ...
        // Do not attach AutoTap's UIWindow context to a global HID event. That
        // directs the event back to the controller app instead of the current
        // foreground target. Sender ID + display-integrated fields are enough.
...
- (BOOL)sendAtX:(CGFloat)x y:(CGFloat)y phase:(ATTouchPhase)phase {
    // Match the proven XXTouch delivery model: event creation and dispatch are
    // serialized on the main run loop through one persistent system client.
    // Hop before taking the lock so pause/stop can never deadlock with it.
    if (!NSThread.isMainThread) {
        __block BOOL sent = NO;
        dispatch_sync(dispatch_get_main_queue(), ^{
            sent = [self sendAtX:x y:y phase:phase];
        });
        return sent;
    }
    @synchronized (self) {
