...
- (BOOL)sendAtX:(CGFloat)x y:(CGFloat)y phase:(ATTouchPhase)phase {
    // Match the working XXTouch delivery model: construct and dispatch every
    // DOWN/MOVE/UP frame on the main run loop through one persistent client.
    // Hop before taking the dispatcher lock to keep lock ordering deterministic.
...
- (void)suspendForDeviceLock {
    if (!NSThread.isMainThread) {
        // Never synchronously wait for UIKit while the scene is locking.
