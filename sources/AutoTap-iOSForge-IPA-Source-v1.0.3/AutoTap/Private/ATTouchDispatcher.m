...
allowWhileLocked:(BOOL)allowWhileLocked {
    ...
    if (!allowWhileLocked && ATSystemOverlayIsDeviceLocked()) { return NO; }
    @synchronized (self) {
    if (!allowWhileLocked && ATSystemOverlayIsDeviceLocked()) { return NO; }
...
- (void)suspendForDeviceLock {
    // A lock can arrive between a synthetic DOWN and UP. Merely clearing the
    // local flag leaves SpringBoard believing a finger is still held on the
    // display, which can make the lock screen and unlock gesture appear frozen.
    // The process-wide lock gate has already blocked every new normal send, so
    // force exactly one final UP through the existing client before releasing
    // it. This method touches only IOHID/CoreFoundation state and is therefore
    // safe to call directly from the Darwin lock notification thread.
    CGFloat releaseX = 0;
    CGFloat releaseY = 0;
    BOOL needsRelease = NO;
    @synchronized (self) {
        needsRelease = _touchActive && _systemClient != NULL;
        releaseX = _lastTouchX;
        releaseY = _lastTouchY;
    }
    if (needsRelease) {
        (void)[self sendAtX:releaseX
                          y:releaseY
                      phase:ATTouchPhaseUp
           allowWhileLocked:YES];
    }
    @synchronized (self) {
        _touchActive = NO;
        _lastTouchX = 0;
