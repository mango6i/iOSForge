...
- (uint32_t)currentWindowContextID;
@end
...
- (BOOL)prepareForDispatch {
    // currentWindowContextID may synchronously hop to the main queue. Resolve
    // it before taking the dispatcher lock; otherwise a main-thread pause can
    // wait for this lock while the worker waits for the main queue, deadlocking
    // both the floating pause button and touch release.
    uint32_t sourceContextID = [self currentWindowContextID];
    @synchronized (self) {
        ...
        if (!_systemClient) {
            self.diagnosticText = @"无法重新创建 HID 系统客户端。";
            return NO;
        }
        ...
        if (_setDigitizerInfo && _sourceContextID != 0) {
            _setDigitizerInfo(neutral, _sourceContextID, 0, 0, NULL, 0, 0);
        }
...
- (uint32_t)currentWindowContextID {
    // HID dispatch runs on the automation queue, while UIWindow and
    // UIApplication state are main-thread-only.  Reading the context ID off
    // the main thread can return zero (or race a scene transition), which
    // makes the first touch appear to do nothing.  Keep the event construction
    // on the worker queue but synchronously obtain only this small routing
    // value on the main thread.
    if (!NSThread.isMainThread) {
        __block uint32_t contextID = 0;
        dispatch_sync(dispatch_get_main_queue(), ^{
            contextID = [self currentWindowContextID];
        });
        return contextID;
    }
    ...
    if (!window) { return _sourceContextID; }
...
- (BOOL)sendAtX:(CGFloat)x y:(CGFloat)y phase:(ATTouchPhase)phase {
    // Never dispatch_sync to the main queue while holding the dispatcher lock.
    // Pause/cancel is initiated on main and also needs this lock.
    uint32_t contextID = [self currentWindowContextID];
    @synchronized (self) {
    ...
    // Position belongs to every frame, including the initial DOWN frame.
    // Range and Touch describe contact transitions; omitting Position on DOWN
    // or keeping the parent out of range makes some iOS versions accept taps
    // but discard all following MOVE frames.
    const uint32_t parentMask = ATDigitizerEventPosition |
        (phase == ATTouchPhaseMove
            ? 0
            : (ATDigitizerEventRange | ATDigitizerEventTouch | ATDigitizerEventIdentity));
    const uint32_t childMask = ATDigitizerEventPosition |
        (phase == ATTouchPhaseMove
            ? 0
    ...
    _setFloatValue(parent, ATFieldDigitizerX, pointX);
    _setFloatValue(parent, ATFieldDigitizerY, pointY);
    _setIntegerValue(parent, ATFieldDigitizerIsDisplayIntegrated, 1);
    _setIntegerValue(parent, ATFieldDigitizerIsBuiltIn, 1);
    _setIntegerValue(parent, ATFieldDigitizerRange, touching ? 1 : 0);
    ...
    if (_setDigitizerInfo && contextID != 0) {
        _setDigitizerInfo(parent, contextID, 0, 0, NULL, 0, 0);
    }
    ATTouchDispatcherRegisterSyntheticTimestamp(timestamp);
    ATTouchDispatcherBeginSyntheticDispatch();
