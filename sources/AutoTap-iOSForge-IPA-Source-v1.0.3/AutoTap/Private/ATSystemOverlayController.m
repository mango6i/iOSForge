...
@interface ATSystemOverlayController () {
    ...
    NSInteger _diagUIKitAccepted;
    NSInteger _diagHIDAccepted;
    ...
    CFTimeInterval _lastOverlayPresentedAt;
...
- (void)handleRecordingUIKitEvent:(UIEvent *)event;
...
// ---------------------------------------------------------------------------
// In-foreground capture path.
//
// While AutoTap itself is the foreground application the system treats its own
// UIKit as the primary receiver of touches, and the private HID monitor only
// gets a coalesced second-hand stream — lifts in particular get merged away
// whenever SwiftUI keeps the main run loop busy. That is exactly why recording
// worked in every other app but dropped taps at random inside AutoTap.
//
// So inside the app we sample from UIKit itself: UIApplication.sendEvent sees
// every touch of this process with complete began/moved/ended transitions and
// never loses a frame. The HID path below stays responsible for the
// background case (AutoTap recording another app), where it is the only
// channel and works reliably.
// ---------------------------------------------------------------------------
static __weak ATSystemOverlayController *_ATActiveRecordingController = nil;

static void ATCaptureUIKitEventForRecording(UIEvent *event) {
    ATSystemOverlayController *recorder = _ATActiveRecordingController;
    if (!recorder || !event) { return; }
    [recorder handleRecordingUIKitEvent:event];
}

@implementation UIApplication (ATRecordingTapCapture)
- (void)at_recordingSendEvent:(UIEvent *)event {
    // After method_exchangeImplementations this slot holds the original
    // implementation, so this call forwards to the real sendEvent:.
    [self at_recordingSendEvent:event];
    ATCaptureUIKitEventForRecording(event);
}
@end
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        Method swizzled = class_getInstanceMethod([UIApplication class], @selector(at_recordingSendEvent:));
...
- (void)handleRecordingHIDEvent:(IOHIDEventRef)event {
    ...
    // Foreground sampling is owned by the UIKit path (see the sendEvent hook
    // above): the HID stream is second-hand here and drops lifts at random.
    // Recording another app keeps using this path, where it is the only
    // channel and delivers every frame.
    ...
    if (!self.recordingCapturesGestures) {
        // Record a tap on lift, the way the 1.0.2 recorder did: DOWN only opens
        // a contact, UP publishes it. Publishing on DOWN instead needed a
        // reliable "this is a new contact" signal, and every signal available
        // here (the event mask bits, or a rising edge while lifts get coalesced
        // away) drops taps at random — the count froze at 0/1/2/3/4/10.
        ...
        if (began) {
            ...
            _recordingTouchLastPoint = point;
            if (hypot(point.x - _recordingTouchStartPoint.x,
                      point.y - _recordingTouchStartPoint.y) > 24.0) {
                _recordingTouchMoved = YES;
            }
        } else if (ended && _recordingTouchCandidate && !_recordingTouchMoved) {
            _diagHIDAccepted += 1;
            [self recordTapImmediatelyAtPoint:_recordingTouchLastPoint time:now];
        }
...
/// UIKit-side capture: only runs while AutoTap is the foreground app, where it
/// is the reliable channel. Mirrors the HID state machine so tap and gesture
/// capture behave identically to the background path.
- (void)handleRecordingUIKitEvent:(UIEvent *)event {
    if (!self.recording || !_recordingActive || _editingSuppressed) { return; }
    if (UIApplication.sharedApplication.applicationState != UIApplicationStateActive) { return; }
    for (UITouch *touch in event.allTouches) {
        CGPoint point = [touch locationInView:nil];
        CFTimeInterval now = touch.timestamp > 0 ? touch.timestamp : CACurrentMediaTime();
        switch (touch.phase) {
            case UITouchPhaseBegan: {
                _recordingPhysicalTouchActive = YES;
                _recordingTouchCandidate = ![self recordingControlOwnsScreenPoint:point];
                _recordingTouchStartPoint = point;
                _recordingTouchLastPoint = point;
                _recordingTouchMoved = NO;
                _recordingTouchStartedAt = now;
                _recordingLastSampleAt = now;
                [_recordingPathPoints removeAllObjects];
                [_recordingPathOffsets removeAllObjects];
                if (_recordingTouchCandidate) {
                    [_recordingPathPoints addObject:[NSValue valueWithCGPoint:point]];
            ...
            case UITouchPhaseMoved: {
                if (!_recordingTouchCandidate) { break; }
                _recordingTouchLastPoint = point;
                CGFloat threshold = self.recordingCapturesGestures ? 12.0 : 24.0;
                if (hypot(point.x - _recordingTouchStartPoint.x,
                          point.y - _recordingTouchStartPoint.y) > threshold) {
                    _recordingTouchMoved = YES;
                }
                if (self.recordingCapturesGestures) {
                    CGPoint previous = _recordingPathPoints.lastObject.CGPointValue;
                    if (_recordingPathPoints.count < 511 &&
                        (hypot(point.x - previous.x, point.y - previous.y) >= 1.5 ||
                         now - _recordingLastSampleAt >= 0.012)) {
                        NSInteger offset = MAX(1, (NSInteger)llround((now - _recordingTouchStartedAt) * 1000.0));
                        [_recordingPathPoints addObject:[NSValue valueWithCGPoint:point]];
                        [_recordingPathOffsets addObject:@(MIN(offset, 10000))];
                        _recordingLastSampleAt = now;
                    }
            ...
            case UITouchPhaseEnded: {
                _recordingTouchLastPoint = point;
                if (_recordingTouchCandidate) {
                    if (!_recordingTouchMoved) {
                        _diagUIKitAccepted += 1;
...
- (NSString *)recordingDiagnostics {
    return [NSString stringWithFormat:
        @"原生%ld(UIK%ld/HID%ld) 数字%lu 点%lu | 开%d/%d 压%d 见%d 角%ld",
        (long)(_diagUIKitAccepted + _diagHIDAccepted),
        (long)_diagUIKitAccepted, (long)_diagHIDAccepted,
        (unsigned long)self.markerHitViews.count,
        (unsigned long)self.points.count,
        self.recording ? 1 : 0,
        _recordingActive ? 1 : 0,
        _editingSuppressed ? 1 : 0,
        self.visible ? 1 : 0,
        (long)_sessionRole];
}

+ (NSString *)activeRecorderDiagnostics {
    ATSystemOverlayController *recorder = _ATActiveRecordingController;
    if (!recorder) {
        // The hook has no registered capture target: sampling cannot run at
        // all until some controller is configured as the active recorder.
        return @"采样器未注册";
    }
    return [recorder recordingDiagnostics];
}
...
- (BOOL)recordingControlOwnsScreenPoint:(CGPoint)point {
    if (!self.hudWindow || self.hudWindow.hidden || _editingSuppressed) { return NO; }
    // A leftover editor backdrop used to swallow every sample of a running
    // take (count stuck at 0). While capture is active there is no point
    // editor by design, so ignore it here.
    if (self.editorBackdrop && !(self.recording && self.recordingActive)) { return YES; }
...
- (void)configureRecordingEnabled:(BOOL)enabled
    ...
    _recordingActive = normalizedActive;
    // Register the one actively capturing controller for the UIKit sendEvent
    // hook; cleared the moment capture stops so the hook never samples a stale
    // session.
    if (normalizedActive) {
        _ATActiveRecordingController = self;
    } else if (_ATActiveRecordingController == self) {
        _ATActiveRecordingController = nil;
    }
    _recordingTouchCandidate = NO;
    _recordingTouchMoved = NO;
    _recordingTouchStartedAt = 0;
...
- (BOOL)showWithPoints:(NSArray<NSValue *> *)points
    ...
    // Remember when the bar appeared: the record button ignores presses during
    // a short grace window so the touch that opened the mode cannot land on it.
    _lastOverlayPresentedAt = CACurrentMediaTime();
    _physicalTouchActive = NO;
    [self resetHIDFilterTouchState];
    _lastPhysicalEventAt = 0;
    [self resetDirectTouchState];
...
- (void)updateWithPoints:(NSArray<NSValue *> *)points
    ...
    // While capturing, the native recorder owns the point list: every lift
    // appends a point synchronously (recordTapImmediatelyAtPoint:) and then
    // syncs a marker. Overwriting it here from Swift — whose draft can lag by
    // one contact, or drop one when a guard trips — deleted the number that was
    // just added, which is why the recorded count froze at random values
    // (0/1/3/4/10…) instead of growing with every tap.
    if (self.recording && self.recordingActive) {
        ...
        if (self.hudWindow != nil) {
            if (self.markerHitViews.count != self.points.count) {
                // A count drift means some view objects were torn down (mode
                // switch, hidden module). In-place syncing cannot rebuild
                // missing markers — force a full rebuild.
...
- (void)toolbarStartRecording:(id)sender {
    // The overlay pops up right under the user's finger when "开启" is tapped,
    // and the delayed HID frames of that very touch can land on the record
    // button — auto-starting a take the user never asked for (the bar then
    // showed a running recorder with nothing recorded). Ignore record presses
    // for a short grace period after the bar appears or reappears.
    if (CACurrentMediaTime() - _lastOverlayPresentedAt < 0.6) { return; }
...
- (void)setEditingSuppressed:(BOOL)suppressed {
    ...
    // Capturing must never be suppressed. Anything that sets this gate while a
    // recorder is running (a stray Settings request, a scene callback) silently
    // stopped sampling and froze the recorded numbers — the count then stopped
    // growing at a random point in the session.
    BOOL effective = suppressed && !(self.recording && self.recordingActive);
    if (suppressed && !effective) {
        self.diagnosticText = @"录制进行中，已忽略悬浮窗隐藏请求。";
    }
    _editingSuppressed = effective;
    if (effective) {
    ...
    self.hudWindow.systemInteractionEnabled = !effective;
    self.hudWindow.userInteractionEnabled = !effective;
    ...
    if (self.hudWindow) {
        self.hudWindow.hidden = NO;
        self.hudController.view.hidden = NO;
        if (!effective) { [self updateOverlayContentInPlace]; }
        [CATransaction begin];
        [CATransaction setDisableActions:YES];
        self.hudController.view.layer.opacity = effective ? 0.0f : 1.0f;
        [CATransaction commit];
    }
    [self refreshHIDFilterSnapshot];
    [CATransaction flush];
...
- (void)hide {
    ...
    _sessionRole = 0;
    // A hidden module must never keep the UIKit capture hook registered on it:
    // that is exactly how taps ended up sampled on a stale duplicate HUD (its
    // callbacks were then dropped for module mismatch) while the session's own
    // overlay showed a count of zero.
    if (_ATActiveRecordingController == self) {
        _ATActiveRecordingController = nil;
    }
    self.registrationGeneration += 1;
    self.completionGeneration += 1;
