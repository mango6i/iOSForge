...
- (void)applyCurrentAppearance {
    ...
    // A nearly-black translucent toolbar became indistinguishable from dark
    // apps while it was being dragged. Use an opaque elevated surface, a
    // contrasting outline and individual button wells so the controls remain
    // visible in both appearances and in a hosted background context.
    ...
    self.toolbar.layer.borderColor = (dark
...
- (BOOL)readPhysicalHIDEvent:(IOHIDEventRef)event point:(CGPoint *)point touching:(BOOL *)touching {
    ...
    CFIndex mask = _getHIDIntegerValue(finger, ATFieldDigitizerEventMask);
    if (finger != event) {
        mask |= _getHIDIntegerValue(event, ATFieldDigitizerEventMask);
    }
    BOOL changesContact = (mask & (ATDigitizerEventRange | ATDigitizerEventTouch)) != 0;
    BOOL changesPositionOnly = (mask & ATDigitizerEventPosition) != 0 && !changesContact;
    // MOVE frames normally carry Position only and omit the current Touch and
    // Range fields. Preserve the existing contact for those frames; only an
    // explicit Range/Touch transition is allowed to end the gesture.
    if (touch || inRange) {
        *touching = YES;
    } else if (changesPositionOnly && _physicalTouchActive) {
        *touching = YES;
    } else {
        *touching = NO;
    }
...
- (void)buildToolbar {
    ...
    panel.alpha = self.running ? 0.20 : 1.0;
