...
/// One opacity the user picks, applied to the whole floating overlay: control
/// bar and numbered markers alike. Running a script no longer fades anything on
/// its own, so the setting is the only thing that decides transparency.
- (CGFloat)controlOpacityOrDefault {
    CGFloat value = self.controlOpacity;
...
- (void)rebuildOverlayContent {
    ...
    for (NSInteger index = 0; index < (NSInteger)self.points.count; index++) {
        ...
        // Numbered markers follow the same user opacity as the control bar; a
        // script that is running no longer fades them on its own.
        marker.alpha = [self controlOpacityOrDefault];
...
- (void)updateOverlayContentInPlace {
    ...
    self.toolbar.alpha = [self controlOpacityOrDefault];
...
- (void)buildToolbar {
    ...
    panel.alpha = [self controlOpacityOrDefault];
