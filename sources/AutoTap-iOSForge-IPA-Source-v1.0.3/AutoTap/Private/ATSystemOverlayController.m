...
- (void)handleRecordingHIDEvent:(IOHIDEventRef)event {
    ...
    if (!self.recordingCapturesGestures) {
        // Detect a new contact from the digitizer state itself, never from the
        // event mask. The Touch/Identity mask bits are not stable across iOS
        // releases and get reshaped when AutoTap is foreground (UIKit coalesces
        // and re-emits frames), so gating sampling on them dropped taps at
        // random — the recorded count stopped at 0/1/2/3/10 for no visible
        // reason. A rising edge on `touching` is a complete tap target on its
        // own, and a contact that travelled far from its start is used as the
        // recovery path for a lift that never arrived.
        BOOL began = touching && !_recordingPhysicalTouchActive;
        BOOL travelledFromStart = touching && _recordingPhysicalTouchActive &&
            hypot(point.x - _recordingTouchStartPoint.x,
                  point.y - _recordingTouchStartPoint.y) > 44.0;
