...
- (void)buildToolbar {
    ...
    if (self.recording) {
        ...
        if (!self.running && self.multiple && self.pointEditingEnabled) {
...
- (void)hide {
    ...
    // Keep one WindowServer context and one global HID client for the process
    // lifetime. Repeated unregister/register and HID client recreation are
    // both private-API races: the next overlay may stay invisible or ignore
    // its first control until the process is relaunched. A logically hidden,
    // transparent layer tree is inert because both filter paths check
    // overlayPresented before claiming any physical input.
    self.visualWindow.hidden = YES;
    self.interactionWindow.hidden = YES;
    self.hudWindow.hidden = NO;
