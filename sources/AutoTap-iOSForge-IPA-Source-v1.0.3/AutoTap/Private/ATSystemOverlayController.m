...
- (void)handlePhysicalHIDEvent:(IOHIDEventRef)event {
    ...
    if (ended) {
        if (_recordingTouchCandidate) {
            ...
            CGFloat endX = MIN(MAX(_lastPhysicalTouchPoint.x / MAX(size.width, 1), 0), 1);
            CGFloat endY = MIN(MAX(_lastPhysicalTouchPoint.y / MAX(size.height, 1), 0), 1);
            BOOL isGesture = hypot(_lastPhysicalTouchPoint.x - _recordingTouchStartPoint.x,
                                   _lastPhysicalTouchPoint.y - _recordingTouchStartPoint.y) > 12.0;
            _recordingLastTapAt = now;
            NSInteger interval = MIN(MAX(intervalMilliseconds, 1), 3600000);
            NSInteger duration = MIN(MAX(durationMilliseconds, 1), 10000);
