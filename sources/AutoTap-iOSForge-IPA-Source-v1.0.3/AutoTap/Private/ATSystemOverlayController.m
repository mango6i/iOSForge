...
- (BOOL)isVisible { return self.overlayPresented; }
...
- (void)applyCurrentAppearance {
    ...
    // Use an opaque elevated surface and individual button wells so the
    // controls remain visible without drawing a box around the floating HUD.
    UIColor *toolbarSurface = [UIColor colorWithWhite:dark ? 0.16 : 1.0 alpha:0.98];
    UIColor *buttonSurface = dark
    ...
    self.toolbar.layer.opacity = (self.running || self.recordingActive) ? 0.20f : 1.0f;
    for (UIView *view in self.toolbar.subviews) {
        if (![view isKindOfClass:UIButton.class]) { continue; }
        UIButton *button = (UIButton *)view;
        button.backgroundColor = buttonSurface;
        button.layer.cornerRadius = MAX(8.0, CGRectGetWidth(button.bounds) * 0.22);
...
- (BOOL)readPhysicalHIDEvent:(IOHIDEventRef)event point:(CGPoint *)point touching:(BOOL *)touching {
    ...
    BOOL explicitFingerRelease = fingerChangesContact && !touch && !inRange;
...
- (void)updateOverlayContentInPlace {
    ...
    self.toolbar.alpha = self.running ? 0.20 : 1.0;
...
- (void)hide {
    ...
    // Keep the registered CAContext alive. Repeatedly unregistering, destroying
    // and recreating a hosted UIWindow races WindowServer and eventually leaves
    // recording mode without a visible toolbar. Closing now only disables and
    // makes the existing context transparent; the next show reuses it.
    self.visualWindow.hidden = YES;
    self.interactionWindow.hidden = YES;
    self.hudWindow.hidden = NO;
