...
@interface ATSwipePathView : UIView
@property (nonatomic) CGPoint startPoint;
@property (nonatomic) CGPoint endPoint;
@property (nonatomic) CGFloat pathScale;
@property (nonatomic) BOOL active;
@property (nonatomic) BOOL selected;
- (void)configureStart:(CGPoint)start
                    end:(CGPoint)end
                  scale:(CGFloat)scale
...
@implementation ATSwipePathView
- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
...
- (void)drawRect:(CGRect)rect {
    ...
    UIColor *color = self.active ? UIColor.systemGreenColor : UIColor.systemOrangeColor;
    CGFloat lineWidth = (self.selected || self.active ? 5.0 : 4.0) * self.pathScale;
    CGFloat unitX = dx / length;
    CGFloat unitY = dy / length;
    CGFloat arrowLength = MIN(22.0 * self.pathScale, MAX(13.0, length * 0.28));
    CGFloat arrowHalfWidth = 8.0 * self.pathScale;
    CGPoint arrowBase = CGPointMake(self.endPoint.x - unitX * arrowLength,
                                    self.endPoint.y - unitY * arrowLength);
    CGPoint left = CGPointMake(arrowBase.x - unitY * arrowHalfWidth,
                               arrowBase.y + unitX * arrowHalfWidth);
    CGPoint right = CGPointMake(arrowBase.x + unitY * arrowHalfWidth,
                                arrowBase.y - unitX * arrowHalfWidth);

    UIBezierPath *line = [UIBezierPath bezierPath];
    [line moveToPoint:self.startPoint];
    [line addLineToPoint:arrowBase];
    ...
    [arrow moveToPoint:self.endPoint];
    ...
    CGFloat startRadius = 6.5 * self.pathScale;
    UIBezierPath *startDot = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(
        self.startPoint.x - startRadius,
        self.startPoint.y - startRadius,
        startRadius * 2.0,
        startRadius * 2.0)];
    [UIColor.systemBackgroundColor setFill];
    [startDot fill];
    startDot.lineWidth = 3.0 * self.pathScale;
    [color setStroke];
    [startDot stroke];
}
@end
...
@interface ATSystemOverlayController () {
    ...
    CGPoint _recordingTouchFarthestPoint;
    CGFloat _recordingTouchMaxDistance;
...
@property (nonatomic, strong) NSMutableArray<ATSwipePathView *> *swipeViews;
...
@property (nonatomic) BOOL gestureRecordingEnabled;
...
// Keep the registered WindowServer context alive for the lifetime of the app.
// Repeated unregister/register cycles can race with WindowServer and make the
// next toolbar intermittently fail to appear.  Visibility is therefore a
// logical state; hiding removes the hosted content instead of destroying the
// context or changing the root layer opacity.
...
- (instancetype)initPrivate {
    ...
    if (self) {
        ...
        _swipeViews = [NSMutableArray array];
...
- (BOOL)isVisible {
    return self.overlayPresented && self.hudWindow != nil && !self.hudWindow.hidden;
}
...
- (void)applyCurrentAppearance {
    ...
    UIColor *solidSurface = [UIColor colorWithWhite:dark ? 0.08 : 1.0 alpha:1.0];
    ...
    self.toolbar.layer.borderColor = UIColor.clearColor.CGColor;
    self.toolbar.layer.shadowOpacity = 0.0;
    // Use UIView.alpha as the single transparency source. Combining alpha
    // with layer.opacity made a running toolbar effectively 4% visible
    // (0.2 x 0.2), and later in-place updates could make an active recording
    // opaque again. One value keeps every mode at the requested 20% opacity.
    self.toolbar.alpha = (self.running || self.recordingActive) ? 0.20 : 1.0;
    self.toolbar.layer.opacity = 1.0f;
    for (UIView *view in self.toolbar.subviews) {
        if (![view isKindOfClass:UIButton.class]) { continue; }
        UIButton *button = (UIButton *)view;
        // The icons themselves are the floating controls.  Do not draw either
        // a shared panel or a separate rounded rectangle around every icon.
        ...
        if ([button.accessibilityIdentifier containsString:@"toolbarClose:"] ||
            [button.accessibilityIdentifier containsString:@"toolbarNoop:"]) {
    ...
    for (ATSwipePathView *swipe in self.swipeViews) {
        [swipe setNeedsDisplay];
    }
...
- (void)stopPhysicalHIDMonitor {
    ...
    _recordingTouchLastPoint = CGPointZero;
    _recordingTouchFarthestPoint = CGPointZero;
    _recordingTouchMaxDistance = 0;
...
- (IOHIDEventRef)fingerEventFromEvent:(IOHIDEventRef)event {
    ...
    IOHIDEventRef contactTransition = NULL;
    IOHIDEventRef positionChange = NULL;
    for (CFIndex index = CFArrayGetCount(children) - 1; index >= 0; index--) {
        IOHIDEventRef child = (IOHIDEventRef)CFArrayGetValueAtIndex(children, index);
        if (!child) { continue; }
        BOOL touch = _getHIDIntegerValue && _getHIDIntegerValue(child, ATFieldDigitizerTouch) != 0;
        BOOL inRange = _getHIDIntegerValue && _getHIDIntegerValue(child, ATFieldDigitizerRange) != 0;
        CFIndex mask = _getHIDIntegerValue
            ? _getHIDIntegerValue(child, ATFieldDigitizerEventMask)
            : 0;
        if (touch || inRange) { return child; }
        if (!contactTransition && (mask & (ATDigitizerEventRange | ATDigitizerEventTouch))) {
            contactTransition = child;
        }
        if (!positionChange && (mask & ATDigitizerEventPosition)) {
            positionChange = child;
        }
    }
    // On lift, Touch and Range are both zero; prefer the child whose mask
    // explicitly reports that contact transition. Movement-only collections
    // otherwise look like a premature finger-up and collapse swipes to taps.
    if (contactTransition) { return contactTransition; }
...
- (void)handlePhysicalHIDEvent:(IOHIDEventRef)event {
    ...
    if (began) {
        // The filter has already claimed this stream before recording decides
        // whether it belongs to the target app. Reuse that decision so a
        // hosted-toolbar drag can never be mistaken for a recorded swipe when
        // UIKit temporarily returns no hit-test result in the background.
        BOOL touchesHUD = _filterTouchActive || [self hudOwnsScreenPoint:point];
        ...
        _recordingTouchFarthestPoint = point;
        _recordingTouchMaxDistance = 0;
        ...
        if (_recordingTouchCandidate) {
            _recordingTouchLastPoint = point;
            CGFloat distance = hypot(point.x - _recordingTouchStartPoint.x,
                                     point.y - _recordingTouchStartPoint.y);
            if (distance > _recordingTouchMaxDistance) {
                _recordingTouchMaxDistance = distance;
                _recordingTouchFarthestPoint = point;
            }
        }
    ...
    if (ended) {
        if (_recordingTouchCandidate) {
            ...
            BOOL isGesture = _recordingTouchMaxDistance > 12.0;
            CGPoint recordedEnd = _recordingTouchLastPoint;
            if (isGesture && hypot(recordedEnd.x - _recordingTouchStartPoint.x,
                                   recordedEnd.y - _recordingTouchStartPoint.y) <= 12.0) {
                recordedEnd = _recordingTouchFarthestPoint;
            }
            CGFloat endX = MIN(MAX(recordedEnd.x / MAX(size.width, 1), 0), 1);
            CGFloat endY = MIN(MAX(recordedEnd.y / MAX(size.height, 1), 0), 1);
            _recordingLastTapAt = now;
            NSInteger interval = MIN(MAX(intervalMilliseconds, 1), 3600000);
            NSInteger duration = isGesture
                ? MIN(MAX(durationMilliseconds, 80), 10000)
                : MIN(MAX(durationMilliseconds, 1), 10000);
            if (isGesture && self.gestureRecordingEnabled && self.recordGestureHandler) {
                self.recordGestureHandler(startX, startY, endX, endY, interval, duration);
            } else if (self.recordTapHandler) {
                self.recordTapHandler(endX, endY, interval, duration);
            }
        }
        _recordingTouchCandidate = NO;
        _recordingTouchMaxDistance = 0;
...
- (BOOL)readPhysicalHIDEvent:(IOHIDEventRef)event point:(CGPoint *)point touching:(BOOL *)touching {
    ...
    if (!finger) { return NO; }
    BOOL touch = _getHIDIntegerValue(finger, ATFieldDigitizerTouch) != 0;
    BOOL inRange = _getHIDIntegerValue(finger, ATFieldDigitizerRange) != 0;
    BOOL parentTouch = finger != event && _getHIDIntegerValue(event, ATFieldDigitizerTouch) != 0;
    CFIndex fingerMask = _getHIDIntegerValue(finger, ATFieldDigitizerEventMask);
    CFIndex parentMask = finger != event
        ? _getHIDIntegerValue(event, ATFieldDigitizerEventMask)
        : 0;
    BOOL fingerChangesContact = (fingerMask & (ATDigitizerEventRange | ATDigitizerEventTouch)) != 0;
    BOOL changesPosition = ((fingerMask | parentMask) & ATDigitizerEventPosition) != 0;
    BOOL explicitFingerRelease = fingerChangesContact && !touch && !inRange && !parentTouch;
    // On several iOS releases the parent collection keeps Range/Touch bits on
    // MOVE frames even though the finger child carries Position only. Merging
    // both masks made that first MOVE look like an UP and collapsed every
    // recorded swipe into a tap. Only the finger child's explicit transition
    // may release an active contact; a position frame preserves it.
    if (touch || parentTouch) {
        *touching = YES;
    } else if (changesPosition && _physicalTouchActive && !explicitFingerRelease) {
        *touching = YES;
    } else {
        *touching = NO;
    }
...
- (BOOL)hudOwnsScreenPoint:(CGPoint)point {
    if (!self.visible || !self.hudWindow || self.hudWindow.hidden || _editingSuppressed) { return NO; }
...
- (UIView *)hudHitViewAtScreenPoint:(CGPoint)point {
    if (!self.visible || !self.hudWindow || self.hudWindow.hidden || _editingSuppressed) { return nil; }
    CGPoint local = [self.hudWindow convertPoint:point fromWindow:nil];
...
- (void)beginDirectTouchAtPoint:(CGPoint)point {
    ...
    if (!self.running && self.toolbar && (hit == self.toolbar || [hit isDescendantOfView:self.toolbar])) {
        _directTouchTarget = ATDirectTouchTargetToolbar;
        _directTargetStartCenter = self.toolbar.center;
        self.gestureInProgress = YES;
        self.toolbar.alpha = 1.0;
        self.toolbar.layer.opacity = 1.0;
        [self.hudController.view bringSubviewToFront:self.toolbar];
    }
}
...
- (void)moveDirectTouchToPoint:(CGPoint)point {
    ...
    if (!self.running && _directTouchTarget == ATDirectTouchTargetControl &&
...
- (void)endDirectTouchAtPoint:(CGPoint)point cancelled:(BOOL)cancelled {
    ...
    } else if (!cancelled && !self.running && target == ATDirectTouchTargetToolbar && moved && self.toolbar) {
        CGRect bounds = self.hudWindow.bounds;
    ...
    if (self.hudWindow != nil) {
        if (self.gestureInProgress) {
            self.rebuildPending = self.rebuildPending || structureChanged;
                   ...
                   structureChanged ||
                   self.markerViews.count != self.points.count ||
                   self.swipeViews.count != self.points.count) {
...
- (void)rebuildOverlayContent {
    ...
    for (NSInteger index = 0; index < self.points.count; index++) {
        ...
        NSDictionary<NSString *, NSNumber *> *settings = index < self.actionSettings.count
            ? self.actionSettings[index]
            : @{};
        BOOL isSwipe = [settings[@"kind"] integerValue] == 1;
        CGPoint end = CGPointMake(
            MIN(MAX([settings[@"endX"] doubleValue] * width, 0), width),
            MIN(MAX([settings[@"endY"] doubleValue] * height, 0), height));
        ATSwipePathView *swipe = [[ATSwipePathView alloc] initWithFrame:bounds];
        [swipe configureStart:CGPointMake(ratio.x * width, ratio.y * height)
                          end:end
                        scale:self.markerScale
                     selected:index == self.selectedIndex
                       active:index == self.activeIndex];
        swipe.hidden = !isSwipe;
        swipe.alpha = (self.running || self.recordingActive) ? 0.20 : 1.0;
        [self.hudController.view addSubview:swipe];
        ...
        [self.swipeViews addObject:swipe];
...
- (void)updateOverlayContentInPlace {
    ...
    for (NSInteger index = 0; index < self.markerViews.count && index < self.points.count; index++) {
        ...
        marker.hidden = isSwipe;
        if (index < self.swipeViews.count) {
            ATSwipePathView *swipe = self.swipeViews[index];
            swipe.frame = bounds;
            [swipe configureStart:CGPointMake(ratio.x * width, ratio.y * height)
                              end:end
                            scale:self.markerScale
...
- (void)buildToolbar {
    ...
    panel.alpha = (self.running || self.recordingActive) ? 0.20 : 1.0;
...
- (void)showCompletionMessage:(NSString *)message {
    ...
    if (!self.visible || !self.hudController.view || self.hudWindow.hidden) { return; }
    ...
    CGRect bounds = self.hudWindow.bounds;
    CGFloat panelWidth = MIN(CGRectGetWidth(bounds) - 64.0, 220.0);
    CGFloat panelHeight = 96.0;
    ...
    panel.layer.borderColor = UIColor.clearColor.CGColor;
    panel.layer.shadowColor = UIColor.blackColor.CGColor;
    panel.layer.shadowOpacity = 0.0;
    panel.layer.shadowRadius = 0.0;
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(12.0, 51.0, panelWidth - 24.0, 28.0)];
...
- (void)showPointEditorAtIndex:(NSInteger)index {
    ...
    // Only single-target scripts have per-target continuous execution.
    // Multi-target and recorded scripts do not receive or save that setting.
...
- (void)registerWindowsWithAttempt:(NSInteger)attempt generation:(NSUInteger)generation {
    ...
    if (attempt < 24) {
        // WindowServer can take more than one second to publish a fresh
        // context while another app is becoming active. Keep retrying long
        // enough that the recording toolbar is not intermittently absent.
...
- (void)setRecordingEnabled:(BOOL)enabled {
    ...
    _recordingTouchStartedAt = 0;
    _recordingLastTapAt = 0;
    if (self.visible) { [self requestDeferredOverlayRebuild]; }
}

- (void)setGestureRecordingEnabled:(BOOL)enabled {
    if (!NSThread.isMainThread) {
        dispatch_async(dispatch_get_main_queue(), ^{ [self setGestureRecordingEnabled:enabled]; });
        return;
    }
    _gestureRecordingEnabled = enabled;
}
...
- (void)hide {
    ...
    [self.countdownBadge removeFromSuperview];
    [self.completionPanel removeFromSuperview];
    // Do not unregister or hide the registered UIWindow. WindowServer releases
    // that context asynchronously, so recreating it immediately after a quick
    // close/open can leave the next toolbar absent. Remove all hosted content
    // instead. The passthrough root then owns no screen points, while the same
    // valid context is ready for the next mode without a registration race.
    for (UIView *view in [self.hudController.view.subviews copy]) {
    ...
    self.recording = NO;
    _recordingActive = NO;
    self.gestureRecordingEnabled = NO;
    _recordingTouchCandidate = NO;
    _recordingTouchLastPoint = CGPointZero;
    _recordingTouchFarthestPoint = CGPointZero;
    _recordingTouchMaxDistance = 0;
