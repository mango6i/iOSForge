...
- (void)syncRecordingMarkers {
    ...
    for (NSInteger index = self.markerHitViews.count; index < self.points.count; index++) {
        ATMarkerView *marker = [[ATMarkerView alloc] initWithFrame:CGRectZero];
        CGFloat hitSide = MAX(58.0, 58.0 * self.markerScale);
        UIView *hit = [[UIView alloc] initWithFrame:CGRectMake(0, 0, hitSide, hitSide)];
        hit.backgroundColor = UIColor.clearColor;
...
- (void)buildToolbar {
    ...
    if (self.recording) {
        ...
        BOOL gestures = self.recordingCapturesGestures;
...
- (void)setGesturePaths:(NSArray<NSArray<NSValue *> *> *)paths
    ...
    for (NSArray<NSValue *> *route in routes) {
        ...
        // Faint full route so "from here to there" stays readable, then the
        // moving indicator on top of it.
        CAShapeLayer *routeLayer = [CAShapeLayer layer];
        routeLayer.frame = bounds;
        routeLayer.path = line.CGPath;
        routeLayer.strokeColor = UIColor.whiteColor.CGColor;
        routeLayer.fillColor = UIColor.clearColor.CGColor;
        routeLayer.lineWidth = 2.0;
        routeLayer.lineCap = kCALineCapRound;
        routeLayer.lineJoin = kCALineJoinRound;
        routeLayer.opacity = 0.18;
        [self.hudController.view.layer addSublayer:routeLayer];
        [self.gesturePathLayers addObject:routeLayer];
        ...
        for (NSInteger dot = 0; dot < ATGestureTrailDotCount; dot++) {
            ...
            // same path and produces the tail.
...
- (void)setRecordingEnabled:(BOOL)enabled {
    ...
    if (!enabled) {
        ...
        // Enabling the recorder only arms it. Capture has to stay off until
        // the red record control is pressed: leaving a previous session's
        // active flag set made opening the mode start recording by itself,
        // which also swapped the record button for a stop button that then
        // looked dead when tapped.
        _recordingActive = NO;
    }
    _recordingTouchCandidate = NO;
    _recordingTouchMoved = NO;
...
- (void)hide {
    ...
    // Keep both the IOHID filter client and the registered WindowServer
    // context alive. Recreating either one after every close is what made the
    // controls and synthetic touches work only after restarting AutoTap.
    // Hiding is purely a presentation state: the existing root layer becomes
    // transparent and non-interactive, then the next show reuses it.
    self.hudWindow.hidden = NO;
