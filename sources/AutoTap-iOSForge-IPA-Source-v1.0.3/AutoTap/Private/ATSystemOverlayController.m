...
- (void)hide {
    ...
    // Keep the reusable UIWindow and HID client, but remove this module's
    // WindowServer registration completely below. Four permanently registered
    // full-screen contexts (even transparent ones) can shadow the next module
    // and remain above the lock screen. A later `show` registers the same live
    // UIWindow context again without sharing toolbar/recorder state.
    self.hudWindow.hidden = NO;
    self.hudWindow.systemInteractionEnabled = NO;
    self.hudWindow.userInteractionEnabled = NO;
    self.visualWindow.hidden = YES;
