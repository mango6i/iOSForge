...
@interface ATSystemOverlayController () {
    ...
    BOOL _awaitingInitialTouchRelease;
...
- (void)applyCurrentAppearance {
    ...
    self.toolbar.layer.shadowOpacity = dark ? 0.34 : 0.18;
    self.toolbar.layer.shadowRadius = 8.0 * MAX(self.controlScale, 0.8);
    self.toolbar.layer.shadowOffset = CGSizeMake(0, 3.0);
...
- (void)handlePhysicalHIDEvent:(IOHIDEventRef)event {
    ...
    // showWithPoints: can be called while the finger that pressed the in-app
    // Enable button is still down. Never let that same physical contact press
    // the newly-created floating Record/Play control. Wait for a complete lift
    // before accepting the first HUD interaction, regardless of tap duration.
    if (_awaitingInitialTouchRelease) {
        if (!touching) {
            _awaitingInitialTouchRelease = NO;
            _physicalTouchActive = NO;
            _filterTouchActive = NO;
            [self resetDirectTouchState];
        }
    ...
    if (CACurrentMediaTime() < _acceptPhysicalInputAfter) { return; }
    ...
    if (began) {
        ...
        if (_recordingTouchCandidate) {
            _recordingTouchLastPoint = point;
            if (hypot(point.x - _recordingTouchStartPoint.x, point.y - _recordingTouchStartPoint.y) > 12.0) {
    ...
    if (ended) {
        if (_recordingTouchCandidate) {
            ...
            BOOL recorded = NO;
            if (_recordingTouchMoved && self.recordingCapturesGestures && self.recordSwipeHandler) {
                self.recordSwipeHandler(startX, startY, endX, endY, safeInterval, MAX(safeDuration, 40));
                recorded = YES;
...
- (uint32_t)processPhysicalHIDEvent:(IOHIDEventRef)event {
    ...
    if (_awaitingInitialTouchRelease) {
        ...
        BOOL touching = NO;
        if ([self readPhysicalHIDEvent:event point:&ignoredPoint touching:&touching] && !touching) {
            _awaitingInitialTouchRelease = NO;
            _physicalTouchActive = NO;
            _filterTouchActive = NO;
            [self resetDirectTouchState];
...
- (BOOL)shouldFilterPhysicalHIDEvent:(IOHIDEventRef)event {
    ...
    if (!self.visible || _editingSuppressed || CACurrentMediaTime() < _acceptPhysicalInputAfter) {
        _filterTouchActive = NO;
        return NO;
    }
    CGPoint point = CGPointZero;
...
- (void)beginDirectTouchAtPoint:(CGPoint)point {
    ...
    if (!self.running && !self.recordingActive && self.toolbar &&
...
- (void)moveDirectTouchToPoint:(CGPoint)point {
    ...
    if (_directTouchTarget == ATDirectTouchTargetControl &&
        _directTouchMoved && !self.running && !self.recordingActive && self.toolbar &&
...
- (BOOL)showWithPoints:(NSArray<NSValue *> *)points
    ...
    _acceptPhysicalInputAfter = CACurrentMediaTime() + 0.18;
    _awaitingInitialTouchRelease = YES;
    _physicalTouchActive = NO;
    _filterTouchActive = NO;
    _recordingTouchCandidate = NO;
    [self resetDirectTouchState];
    ...
    // A lift frame normally clears the hand-off guard above. If the monitor is
    // installed after that frame has already passed, release the guard after a
    // short settling window so the user's first deliberate HUD tap is never
    // swallowed.
    ...
    [self registerWindowsWithAttempt:0 generation:generation];
    return YES;
}
...
- (void)updateWithPoints:(NSArray<NSValue *> *)points
    ...
    if (self.hudWindow != nil) {
        ...
        } else if (structureChanged || self.markerViews.count != self.points.count || self.toolbar == nil) {
...
- (void)updateOverlayContentInPlace {
    ...
    NSString *symbol = (self.running && !self.paused) ? @"pause.fill" : @"play.fill";
    [self.runButton setImage:[UIImage systemImageNamed:symbol withConfiguration:configuration] forState:UIControlStateNormal];
    self.runButton.tintColor = (self.running && !self.paused) ? UIColor.systemOrangeColor : UIColor.systemBlueColor;
...
- (void)buildToolbar {
    ...
    if (self.recording) {
        ...
        BOOL activelyPlaying = self.running && !self.paused;
        [items addObject:@{@"symbol": activelyPlaying ? @"pause.fill" : @"play.fill", @"color": activelyPlaying ? UIColor.systemOrangeColor : UIColor.systemBlueColor, @"action": @"toolbarToggle:"}];
        // Keep the original v1.1 toolbar inventory stable. During playback the
        // toolbar is dimmed and locked, but controls must not disappear and
        // change the panel's shape underneath the user's finger.
    ...
    panel.layer.borderColor = [UIColor.separatorColor colorWithAlphaComponent:0.24].CGColor;
...
- (void)showCompletionMessage:(NSString *)message {
    ...
    CGRect bounds = self.hudWindow.bounds;
    CGFloat panelWidth = MIN(CGRectGetWidth(bounds) - 32.0, 228.0);
...
- (BOOL)acceptToolbarAction:(SEL)action {
    CFTimeInterval now = CACurrentMediaTime();
    // A hosted toolbar can deliver the final physical frame again after its
    // contents are rebuilt. Debounce the whole toolbar, not only one selector:
    // otherwise the same lift can hit Record and then the Stop button that
    // replaces it at the exact same position.
    if (now - _lastToolbarActionAt < 0.45) { return NO; }
...
- (void)toolbarSettings:(id)sender {
    if (self.running || self.recordingActive) { return; }
...
- (void)toolbarFinishRecording:(id)sender {
    // Require a deliberate second press. The start control is replaced by the
    // stop control asynchronously, so an echoed release from the first press
    // must never finish a recording immediately.
...
- (void)registerWindowsWithAttempt:(NSInteger)attempt generation:(NSUInteger)generation {
    ...
    if (hudOK) {
        ...
        // A registered CAContext can otherwise continue presenting the empty
        // layer tree committed by hide until some unrelated UI change occurs.
        // Commit the newly rebuilt controls now and once more on the next main
        // run-loop pass so first-open and reopen behave identically.
        self.hudWindow.hidden = NO;
        self.hudWindow.systemInteractionEnabled = YES;
        self.hudWindow.userInteractionEnabled = YES;
        self.hudController.view.hidden = NO;
...
- (void)setRecordingCapturesGestures:(BOOL)enabled {
    if (!NSThread.isMainThread) {
        dispatch_async(dispatch_get_main_queue(), ^{ [self setRecordingCapturesGestures:enabled]; });
        return;
    }
    _recordingCapturesGestures = enabled;
    _recordingTouchCandidate = NO;
    _recordingTouchMoved = NO;
}
...
- (void)setRecordingActive:(BOOL)active {
    ...
    _recordingTouchStartedAt = 0;
    _recordingLastTapAt = 0;
    _recordingActivatedAt = normalized ? CACurrentMediaTime() : 0;
...
- (void)hide {
    ...
    // Tear down every hosted context as one unit. Reusing a transparent hosted
    // CAContext leaves SpringBoard with stale presentation and hit-test state
    // on some iOS builds: the next toolbar can be visible but untouchable, or
    // fail to appear for gesture playback. A fresh context keeps the visible
    // controls and the raw-HID hit targets in the same generation.
