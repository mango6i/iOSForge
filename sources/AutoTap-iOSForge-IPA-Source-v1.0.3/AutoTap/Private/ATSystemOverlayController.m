...
- (void)hide {
    ...
    // v1.0.2 released the global HID filter whenever a HUD closed. Keeping a
    // filter client alive for every previously opened module leaves several
    // system-wide callbacks scheduled while the app is backgrounded or the
    // device is locking, which can stall SpringBoard input delivery.
    ...
    // Restore the v1.0.2 teardown contract: a closed module owns no registered
    // WindowServer context, no source UIWindow and no HID filter. Reopening a
    // mode constructs a fresh, single context instead of reviving background
    // state from a previous module.
