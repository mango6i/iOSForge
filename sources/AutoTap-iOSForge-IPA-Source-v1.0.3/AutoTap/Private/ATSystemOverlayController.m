...
- (void)handlePhysicalHIDEvent:(IOHIDEventRef)event {
    ...
    if (ended) {
        if (_recordingTouchCandidate) {
            ...
            if (isGesture && self.recordGestureHandler) {
                self.recordGestureHandler(startX, startY, endX, endY, interval, duration);
            } else if (!isGesture && self.recordTapHandler) {
...
- (BOOL)readPhysicalHIDEvent:(IOHIDEventRef)event point:(CGPoint *)point touching:(BOOL *)touching {
    ...
    // Some iOS HID streams only mark Touch on the transition frames while
    // Range remains set for every movement frame. Treat either as an active
    // contact so a physical swipe is not collapsed into a tap.
    *touching = touch || inRange;
