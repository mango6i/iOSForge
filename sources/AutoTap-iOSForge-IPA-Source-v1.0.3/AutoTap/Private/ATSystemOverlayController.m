...
- (uint32_t)processPhysicalHIDEvent:(IOHIDEventRef)event {
    if (!event) { return 0; }
    // Four modules own four long-lived HID clients. An inactive module must
    // return before synchronously entering the main queue; otherwise every tap
    // is serialized through all previously opened modules and rapid switching
    // can delay the final show/registration transition.
    if (ATSystemOverlayIsDeviceLocked() || !_overlayPresented || _editingSuppressed) { return 0; }
    // The system HID client also reports hardware keys. Never synchronize a
    // power/lock-button event onto UIKit's main queue while iOS is suspending
    // the scene; doing so can stall the lock transition. Only digitizer frames
    // belong to the overlay recorder/drag pipeline.
...
- (BOOL)shouldFilterPhysicalHIDEvent:(IOHIDEventRef)event {
    ...
    // Decide ownership once, on the first actual DOWN frame. If a gesture
    // starts in the foreground app and later crosses the HUD, it must stay
    // owned by that app; claiming it halfway through loses the app's UP frame
    // and can leave both the page and the floating controls stuck.
...
- (void)hide {
    ...
    // Keep this module's monitor client alive. Recreating a private HID filter
    // after every close is unreliable on several TrollStore/iOS combinations
    // and was the reason later recording contacts disappeared. Hidden and
    // lock-suspended modules return above before touching the main queue.
    [self stopRecordingHIDMonitor];
    [self dismissPointEditor];
    [self.countdownBadge removeFromSuperview];
    [self.completionPanel removeFromSuperview];
