...
@interface ATSystemOverlayController () {
    ...
    IOHIDEventSystemClientRef _recordingMonitor;
...
static void ATSystemRecordingHIDCallback(void *target, void *refcon, IOHIDServiceClientRef service, IOHIDEventRef event) {
    if (!target || !event) { return; }
    ATSystemOverlayController *controller = (__bridge ATSystemOverlayController *)target;
    if (NSThread.isMainThread) {
        [controller handleRecordingHIDEvent:event];
        return;
    }
    CFRetain(event);
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (BOOL)startRecordingHIDMonitor {
    if (_recordingMonitor) { return YES; }
    // Load the shared IOKit symbols once through the HUD monitor setup. The
    // recording client deliberately uses the observational callback instead
    // of the filter block: it must receive every DOWN/MOVE/UP frame without
    // claiming the foreground app's touch stream.
    if (!_createHIDSystemClient || !_registerHIDCallback || !_scheduleHIDClient) {
        if (![self startPhysicalHIDMonitor]) { return NO; }
    }
    if (!_createHIDSystemClient || !_registerHIDCallback || !_scheduleHIDClient) { return NO; }

    _recordingMonitor = _createHIDSystemClient(kCFAllocatorDefault);
    if (!_recordingMonitor) { return NO; }
    _recordingPhysicalTouchActive = NO;
    _recordingTouchCandidate = NO;
    _recordingTouchMoved = NO;
    _recordingTouchStartedAt = 0;
    _recordingLastTapAt = 0;
    _registerHIDCallback(_recordingMonitor, ATSystemRecordingHIDCallback, (__bridge void *)self, NULL);
    _scheduleHIDClient(_recordingMonitor, CFRunLoopGetMain(), kCFRunLoopCommonModes);
    return YES;
}
...
- (void)stopRecordingHIDMonitor {
    ...
    _recordingTouchStartedAt = 0;
    _recordingLastTapAt = 0;
    if (!_recordingMonitor) { return; }
    if (_unregisterHIDCallback) { _unregisterHIDCallback(_recordingMonitor); }
    if (_unscheduleHIDClient) {
        _unscheduleHIDClient(_recordingMonitor, CFRunLoopGetMain(), kCFRunLoopCommonModes);
    }
    CFRelease(_recordingMonitor);
...
- (IOHIDEventRef)fingerEventFromEvent:(IOHIDEventRef)event {
    ...
    // Child order is not stable across iOS releases. After the first contact,
    // the final child can be an inactive/stale finger, which previously left
    // the recorder permanently waiting for a lift. Prefer an actually touching
    // digitizer child; while tracking, keep the child nearest the last sample.
    ...
    IOHIDEventRef bestTouch = NULL;
    IOHIDEventRef bestFallback = NULL;
    double bestTouchDistance = HUGE_VAL;
    ...
    for (CFIndex index = 0; index < CFArrayGetCount(children); index++) {
        ...
        BOOL touching = _getHIDIntegerValue && _getHIDIntegerValue(child, ATFieldDigitizerTouch) != 0;
        if (touching && distance < bestTouchDistance) {
            bestTouch = child;
    ...
    return bestTouch ?: bestFallback ?: (IOHIDEventRef)CFArrayGetValueAtIndex(children, 0);
}
...
- (void)handlePhysicalHIDEvent:(IOHIDEventRef)event {
    ...
    // Older systems may expose the filter API but not the observational
    // callback needed for the independent recorder. Preserve recording on the
    // shared stream as a compatibility fallback, without double-processing on
    // systems where the dedicated client is active.
    if (!_recordingMonitor && self.recording && self.recordingActive) {
...
- (void)handleRecordingHIDEvent:(IOHIDEventRef)event {
    ...
    if (began) {
        _recordingTouchCandidate = ![self hudOwnsScreenPoint:point];
        ...
        _recordingTouchLastPoint = point;
        if (hypot(point.x - _recordingTouchStartPoint.x, point.y - _recordingTouchStartPoint.y) > 8.0) {
...
- (void)rebuildOverlayContent {
    ...
    for (NSInteger index = 0; index < self.points.count; index++) {
        ...
        marker.alpha = (executing || self.recordingActive) ? 0.20 : 1.0;
...
- (void)updateOverlayContentInPlace {
    ...
    self.toolbar.alpha = (executing || self.recordingActive) ? 0.20 : 1.0;
...
- (void)buildToolbar {
    ...
    panel.alpha = ((self.running && !self.paused) || self.recordingActive) ? 0.20 : 1.0;
...
- (void)showPointEditorAtIndex:(NSInteger)index {
    if ((self.running && !self.paused) || !self.pointEditingEnabled || index < 0 || index >= self.actionSettings.count) { return; }
