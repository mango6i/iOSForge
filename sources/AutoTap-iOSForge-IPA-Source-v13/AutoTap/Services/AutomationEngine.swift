...
final class AutomationEngine: ObservableObject {
    ...
    private func runStep(
        ...
        perform(
            ...
            // A per-target repeat count of zero is intentionally unbounded;
            // it continues until the user pauses or stops the task.
            if action.repeatCount == 0 || nextRepetition < action.repeatCount {
                nextAction = currentIndex
                nextCycle = cycle
                normalizedRepetition = nextRepetition
            } else if currentIndex + 1 < currentActions.count {
                nextAction = currentIndex + 1
                nextCycle = cycle
            ...
            let delay = self.jitteredDelay(
                ...
                percent: currentProfile.timingJitterPercent
            )
    ...
    private func setActiveToken(_ token: UUID?) {
        tokenLock.lock()
        activeToken = token
        if token == nil { activeProfile = nil }
        tokenLock.unlock()
    }

    private func setActiveProfile(_ profile: AutomationProfile) {
        tokenLock.lock()
        activeProfile = profile
        tokenLock.unlock()
    }
