...
- (BOOL)readPhysicalHIDEvent:(IOHIDEventRef)event point:(CGPoint *)point touching:(BOOL *)touching {
    ...
    if (ATTouchDispatcherIsSyntheticTimestamp(_getHIDTimestamp(event))) { return NO; }
...
- (void)setEditingSuppressed:(BOOL)suppressed {
    ...
    // Hide the actual hosted window as well as its root view.  Some
    // WindowServer versions keep a last composited root-view frame visible
    // when only UIView.hidden changes; toggling the existing window avoids a
    // stale marker copy without unregistering or recreating its context.
    self.hudWindow.hidden = suppressed;
    self.hudController.view.hidden = suppressed;
    if (!suppressed && self.hudWindow) {
        self.hudWindow.hidden = NO;
        [self updateOverlayContentInPlace];
    }
    [CATransaction flush];
