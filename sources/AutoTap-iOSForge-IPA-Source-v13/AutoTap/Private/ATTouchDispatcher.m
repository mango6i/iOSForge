...
- (BOOL)prepareForDispatch {
    @synchronized (self) {
        ...
        _sourceContextID = [self currentWindowContextID];
...
- (BOOL)sendAtX:(CGFloat)x y:(CGFloat)y phase:(ATTouchPhase)phase {
    ...
    // IOHID digitizer coordinates are logical screen points.  The overlay and
    // the automation engine already use that same coordinate space; reducing
    // them to 0...1 here makes a target at (390, 844) become (1, 1), so a
    // cross-process click lands in the wrong place.  Clamp only to the active
    // screen bounds and pass the point values through unchanged.
    const IOHIDFloat pointX = MIN(MAX(x, 0.0), MAX(screenSize.width, 1.0));
    const IOHIDFloat pointY = MIN(MAX(y, 0.0), MAX(screenSize.height, 1.0));
