...
- 运行参数：启动倒计时、循环次数；不设置最长运行时间，任务只在达到循环条件或手动暂停/停止时结束；每个编号的间隔、时长和重复次数独立保存。
